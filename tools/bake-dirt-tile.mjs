...
const LIGHT = hex('dirt-0');   // #b09472 乾いた砂
const BASE = hex('dirt-1');    // #8a6b4a 基調
const DARK = hex('dirt-2');    // #6d5236 湿った砂
...
/** ★大きなむら → 中くらいの起伏 → 細かい砂粒、の 3 層 */
const n1 = wrappedNoise(12, 3, rnd);
const n2 = wrappedNoise(40, 8, rnd);
const n3 = wrappedNoise(150, 26, rnd);
...
for (let y = 0; y < H; y += 1) {
  for (let x = 0; x < W; x += 1) {
    ...
    /**
     * ★**整地の筋**（横方向の薄い縞）。
     *   ★ダートは走る前に均されるので、進行方向に沿った浅い筋が残ります。
     *   ⚠️ ★強くすると「布」に見えます。★振幅は 0.03 まで。
     */
    t += Math.sin(v * Math.PI * 2 * 6) * 0.03;
