 ...
 *   ★ここで別の条件で取ると、「その環境の実測分布」ではなくなります。
 *   ⚠️ 🔴 ★上の引用の「本番」は「実物・プレースホルダでない」の意味で、
 *     ★**production のことではありません**（★**NM-1**・2026-09-20）。
 *     ✔ ★実際に測った 71.3% は `docs/pool-staging.json`（staging・2026-08-12）。
 *     🔴 ★production の開放率は **0.3149**で、★一度も確かめていません（★`PROD-NEVER-AGED`）。
 *
 * 【★読むだけです】
