...
export const EXPECTED_EXPOSURE = {
  ...
  story_daily: CLOSED,
  // ── ★`daily_run_log`（★`0052`・2026-09-20 に足しました） ──
  //   🔴 ★**本番へ移行 33 件を当てた直後、★V-20 ③ が本番で赤になりました**
  //     （★`daily_run_log → tools/lib/exposure-registry.mjs に姿を書くこと`）。
  //   ✅ ★**網が働いた形です**: ★新しい表が入ったのに登録簿に無い、を捕まえました。
  //   ✔ ★露出はしていません（★実測: ★付与の全列挙 23 件 に `daily_run_log` は ★**出ません**
  //     ＝ anon / authenticated に 1 つも付いていない）。★**未登録だっただけ**です。
  //   ★中身は日次の実行記録（★`0052`。★ok / detail / rows_written）で、
  //   ★クライアントが読む理由がありません。
  daily_run_log: CLOSED,
  // ── ★`nicks`（★`0054`・2026-09-20・POOL-SUPPLY） ──
  //   ★配合の相性表（★§6.6）。★クライアントが読む理由がありません。
  //   ✅ ★**今回は先に登録しました。** ★2026-09-20、★`0052` の `daily_run_log` を登録し忘れ、
  //     ★**本番へ当てた日に V-20 ③ が落ちました**（★AU-32）。★同じことを繰り返しません。
  nicks: CLOSED,
  // ── ★`training_orders`（★`0057`・2026-09-20） ──
