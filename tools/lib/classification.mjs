...
export const READONLY = [
   ...
   *   🔴 ★`migrate.mjs` の「33 個の OK」は ★**道具がそう言った**だけ。★DB に訊いていません。
   *   ⚠️ ★**「消えた」だけを見ません**（★`CK-14`）: ★`0032` の `revoke all` は
   *     ★剥がしすぎると公開ビューまで閉じるので、★**anon から まだ読めること**を対で見ます。
   *   ★O-7 の本体は `verify-anon-exposure.mjs`（★V-20 の全走査）。★2 つ流すこと。
   *   ⚠️ ★**読むだけ**（★`select` と `has_*_privilege` のみ）。★本番に向けてよい。
   */
  'verify-migration-landed.mjs',
  /**
   * ★**`POOL-SUPPLY` の合否**（★2026-09-20・レビュー側の裁定 ⑥）。
   *   🔴 ★合否は「経路が在ること」ではなく ★**入る数と出る数が釣り合うこと ＋ 有効系統数 ≥ 5**。
   *   ✅ ★供給は ★**製品コードそのもの**（`runBreedingWeek` をそのまま呼ぶ）。
   *   ⚠️ ★**引退は年齢だけ**（★故障と調教は入っていない）→ ★釣り合いは**上振れ側**の評価。
   *   ⚠️ ★**DB に繋ぎません**（★メモリ上の偽のクライアント）。★本番に向ける概念がありません。
   *   ⚠️ ★N は発明していません: ★現役 3 年 × 2 世代 ＝ **6 年**（★D-026 は世代を跨いで効く）。
   */
  'verify-pool-supply.mjs',
  /**
  ...
  /**
   * ★**騎手を消した歩き 8 コマ**を、育成側の画面が読む形で `/art/uma/` に置く（★2026-09-24）。
   *   ★DB を見ない。★書くのは `/art/uma/horse-{walk-NN,walk-sheet,stand,face}.{png,webp}` だけ。
   * ⚠️ ★入力が 8 コマ揃っていなければ ★**1 枚も置きません**（★欠けたコマで歩かせない）。
   * ⚠️ ★毛色は焼き込みません（★画面が `coatCssFilter` で掛ける・★裁定 20260923 §9）。
   */
   ...
   * ⚠️ ★**対照を必ず併記します** — ★既定の `steps(N)` が止まる位置でも測り、大きく出ることを見せます
   *    （★対照が無いと、小さい差が「測り方が緩いから」なのか区別できません）。
   */
  'measure-walk-sprite-steps.mjs',
  /**
   * ★**画面が呼ぶ DB の口が、その環境に在るか**を確かめる（★2026-09-24・手順書 ⑦ の前）。
   *   ★`begin read only` で開き、★1 行も書きません。★本番にも向けられます。
   * 🔴 ★2026-09-24 に ★**画面を先に本番へ出し、口が後**になった（★`/stable/foal`）。★それを止める道具です。
   * ⚠️ ★一覧を手で書きません。★`apps/web/src` の `.rpc('…')` を原文から拾います
   *    （★`side-walk` を数え落とした形を繰り返さないため）。
   * ⚠️ ★**対照を内蔵**しています（★在るはずのない名前を混ぜ、★「在る」と返ったら止まる）。
   * ⚠️ ★引数の形までは見ません（★名前だけ）。★そこは言えないと道具自身が出力します。
   */
  'verify-screen-rpcs-live.mjs',
  /**
   * ★焼いた素材の毛色を、★**画面に出る大きさ**で 1 枚に並べて見る（★2026-09-24）。
   *   ★DB を見ない。★書くのは `out/baked-coats/` の確認用 png だけ。
   * ⚠️ ★合否は ★**等倍**で決めること（★3 倍は原因を探すためだけ）。
   *    ★大きく見せると、★小さいときに読めない絵を「良い」と判断してしまいます。
   */
  'review-baked-coats.mjs',
  /**
   * ★**1 レースを見たときに落ちてくる馬素材の量**を測る（★2026-09-24・レビュー側の条件）。
   *   ★DB にもネットワークにも触れない。★ファイルの大きさを足すだけ（★1 文字も書かない）。
   * ⚠️ ★測るのは ★**repo の大きさではなく、その画面が引く量**です。
   * ⚠️ ★役・型・毛色の決め方は ★**`race/page.tsx` の原文と `@star/render` から**読みます。
   *    ★読めなければ止まります（★既定値で測って「少ない」と嘘を言わないため）。
   */
  'measure-race-asset-bytes.mjs',
  /**
   * ★調教の表情の「部品」（★まぶた・眉だけの透過レイヤー）を受け入れ判定して `/art/uma/` に置く
   *   （★2026-09-24・オーナー決定 D-4）。★DB を見ない。★書くのは `/art/uma/horse-face-part-*.{png,webp}` だけ。
   * ⚠️ ★**1 つでも条件を外したら 1 枚も置きません**（★片方だけ入ると表情が 2 段になります）。
   * ⚠️ ★条件は 3 つ: ★画布が頭部と同じ／★不透明が 10% 以下／★目と眉の辺りに 80% 以上 乗っている。
   *    ★「頭を描いてきた」を落とすための条件です（★実際に 2 回 描き直されました）。
   */
  'publish-uma-face-parts.mjs',
  /**
  ...
  /**
   * ★毛色を馬 ID から引く規則を測るだけ（★2026-09-23・裁定 `REVIEW_HORSE_IDENTITY_VERDICT_20260923.md` §9 条件 ①）。
   *   ★DB にも触らず、★1 文字も書きません（★数を出力するだけ）。
   * ⚠️ ★**分布だけ見ないこと。** ★隣接の組数まで測って、★ハッシュの欠陥を 1 つ捕まえました
   *    （★FNV だけだと末尾しか違わない uuid で上位ビットが揃い、★12 頭立ての隣接が 9.54 組 / 11 組）。
   */
  'measure-coat-distribution.mjs',
  /**
   * ★週送りの周の健康を ★**DB から読むだけ**（★2026-09-24・裁定 §12）。★1 文字も書きません。
   * ⚠️ ★**ワーカーの標準出力は読めません**（★VPS の側）。★「ログに失敗の行が無い」の確認ではなく、
   *    ★**DB に残った跡**を数えるだけです。★道具自身がそう出力します。
   */
  'check-weekly-cycle-health.mjs',
  /**
...
export const STATE_CHANGING = [
   ...
   *
   * 【★読むだけではありません】
   *   ★**⑦ の予行で 1 頭の「生まれた週」を動かします**（★年齢の条件が実際に働くかを見るため）。
   *   ★`beginSandbox` / `endSandbox` で ★**必ず戻し、戻ったことを数えます**（★SB-3）。
   *   🔴 ★レビュー側の指摘: ★**年齢の条件はいまどちらの環境でも 1 頭も落としていません**。
   *      ★働いていない条件は ★**間違っていても誰も気づきません**。★だから予行で働かせて見ます。
   * ⚠️ ★合格線を書き写しません。★**同じ接続で数えた値**と突き合わせます
   *    （★母の数は環境で違うので、★数を書くと環境を変えた日に嘘になります）。
   * ⚠️ ★本番には向けません（`assertNotProduction`）。★本番の数は `diag-initial-parent-pool.mjs` で読みます。
   */
  'verify-initial-breeding-dams.mjs',

  /**
   * ★**出走の取消の受付と確定**を実 DB で確かめる（★2026-09-25・`0079`・D-123）。
   *
   * 【★読むだけではありません】
   *   ★予行の中で ★**レースの段を倒し、★馬に持ち主を付け、★ワーカーの確定まで流します**。
   *   ★`beginSandbox` / `endSandbox` で ★**必ず戻し、戻ったことを数えます**（★SB-3）。
   *   ⚠️ ★`runEntryScratch` は ★**要求ごとに自分で `commit` します**。★`sandboxTx` で包むこと
   *      （★素の接続を渡すと、★予行の取引ごと確定します。★実際に踏みました）。
   * 🔴 ★見るのは ★**「発売の準備に入ったら拒む」**（★D-123 の本体）と、★その対照です。
   */
  'verify-entry-scratch-live.mjs',

  /**
   * ★**staging のオーナーのアカウントに引退馬を渡す**（★2026-09-24・裁定 §13 の第 1 案）。
   *
   * 【★なぜ要るか】
   *   ★登録で渡るのは ★**現役 1 頭**で、★引退まで実時間で約 26 時間。
   *   ★そのままでは `/stable/roles` が空で、★オーナーが一周触れません。
   *
   * 🔴 ★**書きます**（`horses.owner_id` を持ち主の居ない功労馬に立てる）。
   * 🔴 ★**staging 専用**（`assertNotProduction` を呼ぶ）。★本番で同じことをするなら
   *    ★裁定 §13 の第 2 案の条件（★オーナーの直接の指示・功労馬から・前後の頭数を残す）に従うこと。
   */
  'grant-retired-horses-staging.mjs',
