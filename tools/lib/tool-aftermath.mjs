...
export const TOOL_AFTERMATH = {
  ...
  /**
   * ── ★**`SOURCE_MUTATING` の 4 本**（★2026-09-25・裁定 §8 ②）────────────────
   *
   * 🔴 ★**この簿の鍵は ★`STATE_CHANGING` ∪ `SOURCE_MUTATING` になりました。**
   *   ★あちらは ★「DB には触れないが ★**本番ソースを一時的に書き換える**」道具で、
   *   ★簿の註記が ★自分で ★「『必ず戻す』ことを検査で固定してはいません」と書いていました。
   *   🔴 ★それが ★**2 回 起きた**（★2026-09-21・2026-09-25）ので、★要求します。
   *   ⚠️ ★**第 2 の簿は作りません**（★割ると「どちらに載せるか」で漏れる）。
   */
  'build-web.mjs': {
    mode: 'restores',
    why: '★画面を作る（★`npm run build:web`）。★`next build` が ★`apps/web/next-env.d.ts` と '
      + '★`apps/web/tsconfig.json` を ★出力先に合わせて書き換えるので、★写しを取って ★`finally` で戻す。'
      + '🔴 ★**戻し漏れは ただの汚れではない** — ★`tsconfig.json` を commit すると '
      + '★**Vercel が使う `.next` ではない道を指す**（★本番を壊す形）。'
      + '✔ ★対照で確かめた（★`STAR_NEXT_DIST_DIR=.next-staging` で汚れを再現 → ★戻り、★`git diff` 0 行）。'
      + '⚠️ ★`SIGKILL`・電源断では ★`finally` が走らないので ★**汚れたまま残ります**'
      + '（★簿 `NEXT-BUILD-REWRITES-TRACKED-FILES`。★戻し方は ★`git checkout HEAD -- <2 ファイル>`）',
  },
  'inventory/measure.mjs': {
    mode: 'restores',
    why: '★本番エントリ 5 点に ★プローブを 1 行 挿して全テストを回し、★撤去する（N-2 / R-10）。'
      + '✔ ★**撤去後に md5 が挿入前と一致することを検証**し、★一致しなければ非ゼロ終了（:77）。'
      + '★`finally` で必ず撤去する（★テストが落ちても本番コードを汚さない）。'
      + '⚠️ ★2026-09-25 まで ★この簿に項がありませんでした（★母集合が `STATE_CHANGING` だけだったため）',
    countedBy: 'if (md5(after) !== md5(original)) {',
  },
  'mutation/run.mjs': {
    mode: 'restores',
    why: '★ソースを壊して ★「テストが落ちるか」を見て戻す（O-4 / R-11）。'
      + '✔ ★事後条件 (4): ★**復元後に md5 が原本と一致**しなければ ★`toolFailure = true`（:390）。'
      + '✔ ★さらに ★`SIGTERM`/`SIGINT` を捕まえて ★`pendingRestore` から書き戻す（:337）— '
      + '★10 分タイムアウトで ★`POLICY_FIT_WEIGHT = 0` が残り、★次の実行が '
      + '★**「改変なしでテストが落ちている」と誤判定した**実害があったため。'
      + '⚠️ ★それでも ★`SIGKILL` では走りません（★上と同じ限界）',
    countedBy: "if (md5(readFileSync(mut.file, 'utf8')) !== originalHash) {",
  },
  /**
   * 🔴 ⚠️ ★**この 1 本は、★簿の `why` の名乗りが ★実物と食い違っていました。**
   *   ★`SOURCE_MUTATING` の註記は ★「★同上（★ソースを一時的に壊して戻す）」と書いていますが、
   *   ✔ ★**実物は 本体のソースを 1 行も書き換えません**（★2026-09-25 に読んで確かめました）:
   *     ★`git worktree add --detach <WORKTREE> <HEAD_SHA>` で ★**別のツリー**を作り、
   *     ★**その中だけで**改変してゲートを回します（:121・:159）。
   *   → ★だから ★「戻す」対象は ★**本体のファイルではなく、★使い捨ての作業ツリー**です。
   *   ⚠️ ★`countedBy` に「復元の照合」を書けません（★復元する物が無い）。★代わりに
   *     ★**片付けられなかったら止まる**ところを引用します。
   */
  'mutation/gates.mjs': {
    mode: 'restores',
    why: '★較正定数を壊して ★V-ゲートが落ちるかを実測する（Q-P3-42）。'
      + '🔴 ★**本体のソースは書き換えません** — ★`git worktree` で ★別のツリーを作り、★その中だけで壊します'
      + '（★だから本体が汚れていても測る対象は ★HEAD のまま）。'
      + '✔ 🔴 ★**汚していないことを、★機械で数えます**（★これが引用している行）: '
      + '★開始時に ★`git status --porcelain --untracked-files=no` を ★`dirty` に控え（:138）、'
      + '★終わりに同じものを取って ★`after !== dirty` なら '
      + '★「★本体の作業ツリーが汚れています（worktree 分離が効いていません）」で ★`exit 2`（:334）。'
      + '⚠️ ★**「空であること」を要求しません** — ★別の作業で編集しただけでも汚染と誤判定するため、'
      + '★比べるのは ★**開始時の状態**です（★共有ツリーでは この設計でないと使えません）。'
      + '✔ ★片付けは ★**前回の作業ツリーが残っていたら消す**。★消せなければ ★`process.exit(2)` で止まる（:154）。'
      + '⚠️ ★**`--force` を使いません**（★中に何があるか分からないまま再帰削除しない）。'
      + '★`--keep-worktree` を渡したときだけ ★意図して残します。'
      + '⚠️ 🔴 ★**`SOURCE_MUTATING` の `why` の「ソースを一時的に壊して戻す」は、★この 1 本には当たりません。**'
      + '★分類自体は ★（別ツリーであれ）ソースを壊す道具なので ★そのままにしてあります',
    /**
     * 🔴 ★**引用を 2 度 選び直しました**（★2026-09-25 → 09-26・裁定 §9）。
     *
     *   ★① ★`process.exit(2);` … ★**7 か所**あり、★片付けを丸ごと消しても緑（★自分で気づいて捨てた）
     *   ★② ★`前回の作業ツリーを消せません` … ★1 か所だが、
     *     🔴 ★**「片付けが失敗したら止まる」証拠であって、★「本体を汚していない」証拠ではありません。**
     *     ★`:332-340` の照合を ★**丸ごと消しても この文は残る**ので ★緑のままでした
     *     （★レビュー側の指摘・★7 か所のときと ★**同じ弱さが 1 段 薄く残っていた**）。
     *   ★③ ★`if (after !== dirty) {` … ✔ ★**これが「汚していないことを数えた行」**。★ファイル内 ★1 か所（実測）。
     *
     * ⚠️ ★**教訓**: ★引用が ★1 か所であることは ★必要条件にすぎません。
     *    ★**「その行が、名乗っている性質そのものを数えているか」**まで見ること。
     */
    countedBy: 'if (after !== dirty) {',
  },
