...
> ⚠️ 【計測中】と書いた欄は、提出時点で実行が終わっていないものです。数字を予測で埋めていません。
...
| 開始 HEAD | `620e2c1` |
| 終了 HEAD | `620e2c1`（**コミットしていません**。push もしていません） |
...
**【計測中】変更後の 1 本だけを走らせています。結果は終わり次第この節に追記します。**
...
- **前後の比較は変更後の 1 本で取れます。** 変更後の出力には、同じ seed・同じレース・同じ売り目で、切り捨てる前のオッズで払っていた場合の払戻率（本便で足した「切捨て無しなら」の列 ＝ 払戻 ＋ 切り捨てで減った額）が並びます。変更前の `oddsFromProbability` は切り捨て前の値（`min(cap, raw)`）を返していたので、この列が変更前の払戻率に当たります。
  ⚠️ △ 変更前の実行を完走させていないので、「この列が変更前の実行の払戻率と一致する」ことは**実行で照合していません**（式からの推論）。
...
| 13 | `npx tsx apps/cli/src/verify-payout.ts --races 2000 --seeds 42`（変更前は `git stash` を使わず別の作業ツリーで） | 【計測中】§3-3 |
