...
describe('O-4 較正定数が正典・較正条件と一致している', () => {
  it('RACE_RANDOM_K は 0.237（★2026-09-19・オーナー決定・案 2）', () => {
    // ★D-016 で 0.26 が正典化されたが、それは**壊れた頭数分布の下で**求めた値だった。
    //   Q-4 で頭数を正典どおりに直し、案D（裾の厚い混合分布）を入れた条件で再導出した。
    /**
     * 🔴 ★**2026-09-19・オーナー決定（案 2）: 0.22 → 0.237**。
     *   ★出発点 V-4 33.75%（M-9 の後・8 シード）、★傾き −72 pp/単位（seed=42 の 1 本から）、★狙い 32.5%。
     * ⚠️ 🔴 ★**外挿です。★「帯の外 0 本」は未測定**（★確定の 8 シードで初めて数える）。
     * ⚠️ ★正典 §13.1 の表も同じ便で 0.237 に直してあります（★**AU-3**）。
     */
    expect(CALIBRATED_RACE_RANDOM_K).toBe(0.237);
