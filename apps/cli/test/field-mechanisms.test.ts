 ...
 *   | 壊した定数                      | 下流の統計量            | 帯を出たか |
 *   |---------------------------------|-------------------------|-----------|
 *   | `DISTANCE_SUIT_MIN` → 0         | V-6 1.21% → 1.18%（2%） | ★出ない   |
 *   | `OFF_DISTANCE_ENTRY_RATE` → 1.0 | V-6 1.21% → 1.16%（4%） | ★出ない   |
 *   | `OFF_SURFACE_ENTRY_RATE` → 1.0  | V-4 31.29% → 31.29%（0%）| ★出ない  |
 *   | `FIELD_STRENGTH_FLOOR` → 0.0    | V-4 31.29% → 32.81%（6%）| ★出ない   |
 *
 *   ★**下流の統計量はほとんど動きません**（0〜6%）。
 *     `OFF_SURFACE_ENTRY_RATE` に至っては V-4 が**小数第2位まで同じ**です。
 *     つまり V-4 / V-6 は、これらの定数が壊れたことを**まったく見ていません**。
 *
 *   ⚠️ 縮小標本（4,000本）では 5〜15% 動いて見えました。**標本を減らすと動いて見える**ので、
 *      「効いている」の判定も**ゲートと同じ本数**でなければ意味がありません。
 *
