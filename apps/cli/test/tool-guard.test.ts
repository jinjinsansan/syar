 ...
 *
 * 【🔴 ★2026-09-25: ★**「版管理にある」から「配られうるもの」に変えました**】
 *   ★裁定 `REVIEW_OWNER_SCOPE_AND_STUD_FEE_20260925.md` §8 ①・簿 `TOOLS-UNCLASSIFIED-WHILE-UNTRACKED`
 *
 *   ⚠️ ★上の註記は ★**「穴は残ります: `git add` するまでは分類されなくても通ります」**と
 *      ★**自分で書いていました**。★「そこを塞ぐのが CI」とも書いていました。
 *   🔴 ★**塞げていませんでした。** ★2026-09-25、★予行と診断の道具 ★**12 本**が
 *     ★**1 日 のあいだ分類簿の外**に居ました。★うち 4 本は ★**staging に書き込む道具**です。
 *     ★CI は ★push されるまで走らないので、★**手元で 1 日 使うぶんは 誰も見ません**。
 *   → ★★**註記に書いた穴は、★註記では塞がりません**（★R-29: ★作法は忘れられる）。
 *
 * 【★なぜ ★`--others --exclude-standard` を足すのか】
 *   ★`--others` … ★**未追跡**も入れる（★作った時点で分類が要る）
 *   ★`--exclude-standard` … 🔴 ★**`.gitignore` されたものは入れない**（★下書きの道を消さない）
 *   ✔ ★実測（2026-09-25・両者が独立に measured・一致）:
 *     ★追跡のみ **363** ／ ★追跡＋未追跡（無視を除く）**363**（★差分 0 ＝ 赤くならずに穴だけ塞がる）
 *     ★無視されている下書き **114 本**は ★**1 本も入りません**
 *   → ★★**手元が CI より厳しくなる向き**なので、★上の CI-3（★手元が緑で CI が赤）は起きません。
 *
 * ⚠️ ★下書きを置く道は ★`.gitignore` の ★**`tools/_*`**（★2026-09-25 に `tools/_*.mjs` から広げました。
 *    ★下書きが `.ts`/`.sh` のときも道が要るため。★`_` で始まる追跡ファイルは ★**0 件**を確認済み）。
 * ⚠️ ★`git` が無い環境では ★**投げます**（★空の一覧で緑にしない・R-21）。
 */
function trackedToolFiles(): string[] {
  /**
   * ★**配られうるもの**（★追跡 ＋ ★無視されていない未追跡）。
   * ⚠️ ★`--cached` を省くと ★追跡ぶんが落ちます（★`--others` だけでは未追跡しか出ません）。
   */
  const cmd = 'git ls-files --cached --others --exclude-standard tools';
  const out = execSync(cmd, { cwd: ROOT, encoding: 'utf8' }).trim();
  if (out === '') throw new Error(`★\`${cmd}\` が空です（★走査が壊れています・R-21）`);
