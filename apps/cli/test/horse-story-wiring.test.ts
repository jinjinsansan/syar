...
describe('★馬物語帳の配線（§18・D-108）', () => {
  ...
  /**
   * ⑤ ★**持ち主の個人情報を出していない**（★牧場名まで・LR-6）
   *
   * 🔴 ★**2026-09-25 に正の側を緩めました。** ★理由を書きます。
   *   ★それまでは ★`stableName` が画面に在ることを要求していました（★他人の馬の行に出す値）。
   *   ★同じ日に ★この画面を ★**見本のデータから本物へ繋ぎ**（★裁定 §5）、
   *   ★`my_retired_horses()` が ★**自分の分だけ**返すので ★他人の馬の行を出せなくなりました
   *   （★公開の引退馬の一覧の口が ★まだ在りません）。
   *   ★見本の行を混ぜれば `stableName` は残せますが、★**本物だと思わせる**ので採りません。
   *
   * ⚠️ ★**負の側（個人情報を出さない）は 1 つも緩めていません。**
   * ⚠️ ★他人の馬を並べ始めたら ★**`stableName` が必須に戻ります**（★下の分岐がそれを言います）。
   */
  it('⑤ ★持ち主の個人情報を出していない（★牧場名まで・LR-6）', () => {
    /** ★他人の馬を「まだ出せない」と画面で言っているか */
    const saysNotYet = CODE.includes('いまは自分の馬だけ出ます');
    if (!saysNotYet) {
      // ★他人の馬を出しているなら、★牧場名までに留めていること
      expect(CODE, '🔴 ★他人の馬を出すなら `stableName` を使うこと（★表示名を出さない）')
        .toMatch(/stableName/);
    }
