...
describe('NT-3 開いている指摘の登録簿', () => {
  ...
  it('★監査から来た指摘が、★**開いている間は簿に居る**（★この簿を作った理由そのもの）', () => {
     ...
     */
    for (const id of ['AUDIT-TLS', 'AUDIT-ZIP', 'AUDIT-CLAUDE-MD']) {
  ...
  it('🔴 ★★片付いたものが簿に戻ってきていない（★「直した」の記録）', () => {
    ...
    const settled: Readonly<Record<string, string>> = {
      ...
      /**
       * 🔴 ★**格下げのうえ、記載の穴を埋めて閉じた**（2026-09-19）。
       *   ★監査の主張（★公正性が壊れる）は ★**偽**でした（★`fairness.ts:102` は行から読む・
       *   ★`pg-store.ts:189-196` は同じ 1 本の insert）。★残っていたのは ★**記載の穴だけ**。
       * ⚠️ ★そして ★**監査の「両方に無い」は、★片方だけ正しかった** —
       *   ★`.env.example:43` には ★**既に在りました**（★`grep -c` で 1 / 0）。
       */
      'M-4': '★`deploy/README.md` §3 に `STAR_SEED_SECRET=` と節を足した（★任意である理由・'
        + '★公正性が壊れない根拠・★fail-closed にしない理由）。★`.env.example` には元から在った',
      /**
      ...
      'CALIB-MARGIN': '★`RACE_RANDOM_K` 0.22 → **0.237**（オーナー決定・案 2・`db85cfe`）の後、'
        ...
        + '★V-4 **0/8**・V-5 **0/8**・V-6 **0/8**（★動かす前は V-4 2/8・V-5 1/8）。'
        + '★プール値 V-4 32.3% / V-5 62.6% / V-6 1.18% — ★3 つとも PASS。'
        + '★→ ★**余裕を足す必要は無い**（★条件が「0 でなければ」なので）',
      'TL-1': '★状態を変える道具 **47 本**を `restores` / `consumes` / `pending` の 3 つに分類し、'
        + '★**`pending` を 16 → 0** にした（2026-09-19）。★`restores` は戻ったことを数え、'
        + '★`consumes` は何を消費するかを宣言し、★どちらも **0 でなければ落ちる**。'
        + '★数え方は `tools/lib/leftovers.mjs` に 1 か所へ集約。'
        + '⚠️ 🔴 ★**「数えるようにした」であって「実 DB で確かめた」ではない** — '
        + '★どの道具も staging で流し直していない。★そこは各道具の註記に印つきで残してある。'
        + '⚠️ ★`tool-aftermath.test.ts` のラチェットが 0 なので、★**新しい道具は最初から数えること**',
      'AUDIT-CANON-BASE-GAIN': '★レビュー側が `7a577a2` で正典 §13.1 を直した'
        + '（`BASE_GAIN` 12→7.8 ／ `INJURY_BASE` 0.0018→0.0013 ＋ ★AU-3「この表の読み方」）。'
        + '★2026-09-19 に `STAR_SPEC_v2.0.md:1729,1733` を読んで確かめた。'
        + '🔴 ★ただし同じコミットが `:1755` を壊している（★別途 報告済み・★正典の修復はレビュー側）',
