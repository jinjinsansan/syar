...
export const EXEMPT: readonly { key: string; why: string }[] = [
  ...
  { key: 'STAR_MIN', why: '★尺度の下限（1.0）。★**D-114 で画面に出なくなった後も 1.0〜5.0 の尺度を保っている** — ★`priceOfStars = max(3000, ★×2000)` が★を読むので、★段番号（1〜24）に変えると馬の価格が最大 4.8 倍になる。★価格を戦績から決め直すのは T-11 の仕事' },
  { key: 'STAR_MAX', why: '★尺度の上限（5.0）。★下限と同じ理由で保っている（`STAR_MIN` の項）' },
  { key: 'STAR_STEP', why: '★1 段ぶんの尺度の幅。★**導出値**（(5−1)÷23 ≈ 0.174）で、★直接置いた数ではない。★旧 0.5（半星）から、★24 段化に伴って自動で細かくなる' },
  ...
  { key: 'STAR_PRICE_EP', why: '★馬の購入の★1 つあたりの価格 [EP]（D-102 ④）。★較正定数だが**値そのものをゲートにしない**（指示書 GB-4）。GB-6 の収支の取り直しで、この額込みの 1 キャリアの収支と「同じ EP での期待する★」を報告する' },
  ...
  { key: 'MARKET_STOCK_MIN', why: '★NPC の現役プールの在庫の下限（D-102 ⑤・D-079 ⑧）。★割ったら警報を返す値で、★黙って帯を広げないための線' },
  { key: 'LISTED_BANDS', why: '★出品する帯（★**段番号**）。★較正値ではなく**品揃えの決め**（2026-09-16・第 5 便-2）。★上（最上位）を出さないのは「配合で狙うもの」を買えてしまい §1.1 が薄まるため、★下を出さないのは誰も買わないため。★動かすと買える帯が変わるだけで、★着順にも成長にも入らない。⚠️ ★**2026-09-18・T-10（D-114）で書き直した** — ★旧 `[2.0,2.5,3.0,3.5,4.0]`（★9 段のうち 5 帯）を、★**24 段のうち旧の値にいちばん近い段** `[6,9,12,14,17]` にした（★口数 15 口も価格の水準 4,087〜7,913 EP も変えていない）。★**段 6〜17 を全部出すと 36 口**になり D-102 ③「選び直しが成立しない」に近づくので、★帯の数を 5 のまま保った。🔴 ★**T-11 でこの仕組みごと消える**（★候補は「走った実績のある馬」・価格は §10.5 の式）' },
  { key: 'LISTINGS_PER_BAND', why: '★帯ごとに出しておく口数（3）。★較正値ではなく**品揃えの決め**。★多くすると「選び直し」に近づく（D-102 ③「振り直しが成立しない」）ので、★通すために増やせる値ではない' },
