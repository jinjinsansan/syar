...
export const EXEMPT: readonly { key: string; why: string }[] = [
  ...
  { key: 'STAR_THRESHOLDS', why: '★素質の平均 → ★（1〜5・0.5 刻み）の境目。★較正値ではなく**見せ方の刻み**で、能力・着順・成長には入らない。⚠️ ★この帯は D-102 ③ の「★表示が同じ帯から候補を出す」の定義でもあるので、★動かすと購入の候補の帯も一緒に動く（★見た目と中身の帯を離さないため、★1 か所にしてある）' },
  { key: 'STAR_MIN', why: '★★の下限（1.0）。正典 §5.5 の星表示の範囲の写し' },
  { key: 'STAR_MAX', why: '★★の上限（5.0）。正典 §5.5 の星表示の範囲の写し' },
  { key: 'STAR_STEP', why: '★★の刻み（0.5・半星）。正典 §5.5 と画面の `Stars` の写し' },
  ...
  { key: 'LISTED_BANDS', why: '★出品する★の帯（2.0〜4.0）。★較正値ではなく**品揃えの決め**（2026-09-16・第 5 便-2）。★5.0 を出さないのは「配合で狙うもの」を買えてしまい §1.1 が薄まるため、★1.0〜1.5 を出さないのは誰も買わないため。★動かすと買える帯が変わるだけで、★着順にも成長にも入らない' },
  ...
  { key: 'WEEK_REAL_MS', why: '★1 ゲーム内週の実時間 4 時間（正典 D-007）。★サイクル長を変えても動かさない値で、CYCLES_PER_WEEK はここから導く。'
    + '★較正で動かす値ではなくゲームの構造' },
