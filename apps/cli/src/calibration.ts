...
export const EXEMPT: readonly { key: string; why: string }[] = [
  ...
  { key: 'PRICE_TIERS_EP', why: '★出品を並べる★**価格の帯の下限** [EP]（★**T11-1 ②**・2026-09-19）。★較正値ではなく**品揃えの決め**で、★着順にも成長にも入らない。🔴 ★**T-11 で `LISTED_BANDS`（素質の帯）から差し替えた** — ★棚を素質の帯で並べると★「どの棚にいるか」が帯を教え、★D-114 ② が塞いだ口が開き直す（★逆算の復活・D-102 ③）。⚠️ 🔴 ★**この境目はまだ測って決めていません** — ★staging の実測（2026-09-19）では★候補 732 頭の価格が **3,000〜4,620 EP** に固まり、★**5 段のうち 3 段が空**です。★照会中（`QUESTIONS_T11_20260919.md`）' },
