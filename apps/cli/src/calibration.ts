...
export const SCAN_EXCLUDED_FILES: readonly { file: string; why: string }[] = [
  ...
  {
    file: 'apps/cli/src/ep-budget.ts',
    why: '★EP の出入りを計算して印刷するだけの道具（★2026-09-25・照会 `QUESTIONS_EP_BUDGET_20260925.md`）。'
      + '🔴 ★**自前の数を 1 つも持ちません** — ★入りは `EP_GRANTS`（`@star/betting`）、'
      + '★1 年の実日数は `WEEKS_PER_YEAR` / `WEEKS_PER_DAY`（`@star/scheduler`）、'
      + '★調教の額は `MENUS`（`@star/training`）、★種付料は `STUD_FEE_*`（`@star/scheduler`）から引いています。'
...
export const EXEMPT: readonly { key: string; why: string }[] = [
  {
    key: 'DAY_BOUNDARY_STALE_AFTER_HOURS',
    why: '★`world_state.day_started_at` を「古い」と呼ぶ線 28 時間'
      + '（`packages/scheduler/src/programme.ts`・`0081_day_boundary_staleness.sql`・裁定 §7 ①・2026-09-25）。'
      + '🔴 ★**較正定数ではありません** — ★「古い」と呼ぶ線です。★ゲームの挙動（着順・オッズ・能力・成長）には入りません。'
      + '★根拠: ★正常時 `day_started_at` は 0〜24 時間前（`DAY_MS`）なので、24 時間を超えたら 1 回 進みそこねている。'
      + '★ワーカーは 6 分ごとに書くので境目直後は 24 時間 + 数分に届きえる → ★余裕 4 時間で 28。'
      + '⚠ ★上げると止まっているのを隅し、★下げると正常な境目直後に鳴る（★通すために動かせる値ではない）。'
      + '★正は SQL の `day_boundary_stale_after_hours()` で、★`ep-grant-sql.test.ts` が突き合わせる',
  },
  {
    key: 'REBUILD_PINS',
    why: '★作り直す画面が「いま何を呼んでいるか」の釘（`apps/cli/src/screen-rebuild-pins.ts`・'
      + '裁定 `REVIEW_SCREEN_GENERATIONS_20260925.md`・2026-09-25）。'
      + '🔴 ★**較正定数ではありません** — ★中身は ★RPC の名前と `lib/` の名前の並びで、★数ではありません。'
      + '★ゲームの挙動には入りません。'
      + '★在る理由: ★2026-09-25 に `/training` → `/train` の転送で '
      + '★**`rpc(\'set_training_order\')` を呼ぶ唯一の画面を消しかけました**（★育成のループが止まる）。'
  ...
  },
  {
    ...
    why: '★旧世代の画面の枚数の上限（`apps/cli/src/screen-generations.ts`・裁定 '
      ...
      + '🔴 ★**較正定数ではありません** — ★**釘**です（★増えたら落ちる／減らすのは歓迎）。'
      + '★ゲームの挙動（★着順・オッズ・能力・成長・ポイント）には 1 ビットも入りません。'
      + '⚠️ ★この数は ★**そのつど実測して書くこと**。★2026-09-25 に ★**引き算で 2 度 間違えました**'
      + '（★37 − 2 ＝ 35 と書いて実測 36 で落ち、★次に `/training` を戻したのに 36 のままにして実測 37）',
  },
  {
    key: 'CANON_AMOUNTS',
    why: '★正典が額を決めた項目 ↔ その額を動かす実装の **対応表**（`apps/cli/src/canon-amounts.ts`・'
      + '裁定 `REVIEW_EP_INFLOW_AND_ENTRY_20260925.md` §7 ③）。'
      + '🔴 ★**較正定数ではありません** — ★ここには **額を 1 つも書いていません**（★額は言葉で書き、'
      + '★数の突き合わせは `ep-grant-sql.test.ts` の仕事）。★見ているのは ★**渡す側／引く側が在るか**だけです。'
      + '★D-075 のデイリー 200 EP が ★**1 か月以上「決めただけで渡す側が無い」ままで、本番の残高が 0 になった**のを受けて作った。'
      + '★動かしてもゲームの挙動（着順・オッズ・能力）は 1 ビットも動かない',
  },
  {
  ...
  },
  {
    ...
    why: '★`apps/cli/src/growth-tell-frequency.ts` が振る**閾値の候補**（★**GB-1 ④**・2026-09-19）。'
      ...
      + '★ゲームの挙動に 1 ビットも入りません（★製品が読むのは `GROWTH_TELL_MIN` だけ）。'
      + '★ここを増やしても減らしても、★変わるのは**表の行数**だけです。'
      + '⚠️ ★ただし ★**いまの `GROWTH_TELL_MIN` が候補に入っていること**は検査が見ます'
      + '（`growth-tell-frequency.test.ts`）— ★候補から外すと、★回数を測れなくなるためです',
  },
  {
  ...
  { key: 'GROWTH_TELL_MIN', why: '★「前より○○できるようになった」と言い始める伸びの幅（D-116 ③）。★較正定数だが**値そのものをゲートにしない**。'
    + '🔴 ★閾値が要るのは、★**毎週何かを言い続けると符号列が積み上がり**、★十分長く観測すれば「どの能力が何回伸びたか」が分かるため'
    + '（★D-114 が「代償を承知で採る」と書いた漏れを**わざわざ広げる**ことになる）。'
    + '✅ ★**2026-09-19・GB-1 ④ で 8 → 13**（★オーナー決定）。★決め方は「閾値から決めず、★**一生で何回 言うか**から決める。推しは 20〜30 回」。'
    + '✔ ★測った（`apps/cli/src/growth-tell-frequency.ts`・150 頭 × 8 シード ＝ 1,200 頭）: ★幅 **8 → 37.6 回**（帯に収まる馬 2%）／★**13 → 24.4 回（98%）**／16 → 20.2 回（68%）。'
    + '★13 が平均（帯の真ん中 25 に最も近い）も収まる割合も最良。★**一生で 1 度も言われない馬は 0%**。'
    + '🔴 ★**比べる相手は「前に言ったときの `stats`」**（★累積）。★週次だと**一生に 1.2 回**しか言われず、★どの幅でも 20〜30 に届かない。'
    + '⚠️ ★**値の照合では守らない**（R-14）。★`apps/cli/test/growth-tell-frequency.test.ts` が**回数を測って帯の中か**を見る — ★成長の較正が動けばそこが落ちる' },
  ...
  { key: 'CAREER_ASSUMPTION', why: '★1 頭・1 キャリアの収支を並べ直すときの前提（現役 90 週・★**出走数は `CAREER_RACE_LIMIT` を引く**・登録料 200 EP・配合費 2,000 EP・賞金 25,000/120,000 PP 相当）。'
    + '🔴 ★**2026-09-19 まで `starts: 24` を直書き**していた（D-052）。★CC-1 ③ で 24 → 40 になったとき、★ここだけ古くなるところだった。'
    + '⚠️ ★**したがって §3.4 の収支は取り直しが要る**（★24 戦の前提で書かれた表）。'
    + '⚠️ 🔴 ★**`weeks: 90` は正典 D-007 の「現役 156 週」と食い違っています** — ★別便で確かめること（★2026-09-19 に気づいた）。★**正典 §3.4 の表の前提の写し**であって較正値ではない（★同じ仮定のまま、D-102 馬の購入・D-103 厩舎の格・D-105 騎手の料金を足して取り直すために置いた・GB-6）。★調教費・騎手の料金・購入価格は写さず、実装から引いている（`economy-balance.test.ts` ① が二重帳簿を捕まえる）' },
  ...
  { key: 'EP_GRANTS', why: "★EP の発行の額（登録時 2,000・デイリー 200・日次上限 10,000）。★正典 **D-075** の写しで、★D-075 自身が「較正定数であり、V-11（ポイント純発行量）を満たすように調整する。★値そのものをゲートにしない」と書いている。★渡すのはサーバー（`0080` の `ep_grant_amount()`）で、★**RPC は額を引数で受け取らない**（★利用者が発行量を決められる形にしない）。★ここと SQL の 2 か所にあるので、★`apps/cli/test/ep-grant-sql.test.ts` が突き合わせる（★ずれたら落ちる）。⚠ ★日次上限は **残高の上限でなく「発行」量の上限**で、★何を発行と数えるかは `ep_reason_class()` が 1 か所で持つ" },
  ...
  { key: 'WEEKS_PER_YEAR', why: '★1 年 ＝ 52 週。★暦の事実であって較正値ではない（動かすと「歳」の意味が変わる）。'
    + '★2026-09-20・案 B-3（生まれた週を 52 週へ層化して配る）で置いた。'
    + '🔴 ★導出にしていない理由: ★正典 §7.1 の節目（104 ＝ 2 歳・260 ＝ 5 歳末）から 52 を割り戻すと、'
    + '★**節目を動かした日に「1 年」の長さが黙って変わります**。★直書きし、★`assertYearAligned()` が'
    + '★「節目が 52 で割り切れるか」を見張る形にした（★割り切れなくなったら投げる・CYCLES_PER_WEEK と逆の作法）' },
  { key: 'CAREER_RACE_LIMIT', why: '正典 §7.1「キャリア上限」の写し。★**2026-09-19・CC-1 ③ で 24 → 40**（オーナー決定）。'
    + '✔ ★理由: 上級条件が資格者不足で 4〜5 割 中止になっていた（PO-6/CC-4）。'
    + '★プールの大きさは効かず（PO-7: 3,000 でも 7,333 でも「4 勝以上」は 7.7 頭で ±0）、★キャリア長が梃子だった'
    + '（24 → 40 で成立率 86.2% → 99.1%・上級条件の開催が 2,121 → 3,548 本）。'
    + '🔴 ★写しをもう 1 か所に作らないこと — ★`economy-balance.ts` が `starts: 24` と直書きしていたのを同じ便で直した（D-052）' },
  {
    key: 'SELECTED',
  ...
  {
    key: 'ANNOUNCE_AHEAD_BY_GRADE',
    why: '★**格ごとの、告知を先に出すサイクル数**（★**DS-5 ②**・2026-09-19・オーナー決定）。'
      + '★較正値ではなく**登録の窓の長さの決め**で、★着順にも経済にも入らない（★いつ知らせるかだけ）。'
      + '★**G1 = `CYCLES_PER_WEEK`（40 サイクル ＝ 4 時間 ＝ 1 ゲーム内週）** → ★登録できる長さは締切まで **38 サイクル ＝ 3 時間 48 分**。'
      + '★**G2・G3 は `ANNOUNCE_AHEAD_RACES` のまま（12 分）** — ★オーナーの承認の文に挙がっているのは G1 だけなので、★狭いほうへ倒した（R-27）。'
      + '★理由: ★**G1 は一生（26 日）に 11.1 本しか開かれない**のに、★12 分前にしか登録できず「今度の G1 に合わせて仕上げる」が成立しなかった。'
      + '★★**狙って出られる回が 0 → 全部**になる。'
      + '🔴 ★**40 を数で書かず `CYCLES_PER_WEEK` から引く** — ★D-007 はサイクル長を 10 分 → 3 分 → 6 分と動かしており、★数で書くと「1 週前」という意味のほうが壊れる。'
      + '✔ ★**画面に並ぶ本数を測った**（5,000 サイクル）: ★1 周 平均 4.065 本・**最大 5 本**（★6 本は一度も出ない）。★4 本のままが 93.5%、★5 本が 6.5%。'
      + '⚠️ ★**DS-5 ③**: ★窓が 1 週の 95% になったので、★**登録と発走の間に週送りが入るのが常態**になる（★12 分のときは 10%）。★D-111 ⑥（引退による取消と返金）が G1 では常用の経路になる',
  },
  {
  ...
  {
    key: 'PLAYER_NAME_MIN_CHARS',
    why: '★利用者が付ける馬名の最少文字数（2）。★較正定数ではなく**正典 D-120「命名の規則（暫定）」の写し**'
      + '（★裁定 `REVIEW_I3_NAMING_VERDICT_20260922.md` §4）。★着順・経済に効かない。★変えるのは正典の改訂のときだけ',
  },
  {
    key: 'PLAYER_NAME_MAX_CHARS',
    why: '★利用者が付ける馬名の最多文字数（9）。★較正定数ではなく**正典 D-120「命名の規則（暫定）」の写し**'
      + '（★裁定 `REVIEW_I3_NAMING_VERDICT_20260922.md` §4）。★着順・経済に効かない。★変えるのは正典の改訂のときだけ',
  },
  {
