...
export function generateRace(
   ...
   *
   * ★上限を超えた登録は ★**ここに来る前に抽選で絞られています**
   *   （★正典 §10.4:1321「上限超過は**完全抽選**」・`apps/worker/src/entry-lottery.ts`）。
   * ⚠️ ★したがって下の `throw` は ★**届かないはずの番人**です。
   *    ★届いたら「抽選を通していない呼び方がある」ということなので、★**黙って切り捨てません**
   *    （★ここで先頭 18 頭を取ると、★**賞金上位優先と同じ偏り**が登録順で入ります）。
   */
  const forced = opts.mustInclude ?? [];
  if (forced.length > sizeMax) {
    throw new Error(
      `generateRace: 必ず出走させる馬が ${forced.length} 頭で、上限 ${sizeMax} 頭を超えています`
        + '（★呼ぶ側が §10.4 の完全抽選を通していません・entry-lottery.ts）',
    );
  }
