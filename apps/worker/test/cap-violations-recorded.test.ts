...
describe('🔴 ★O-3: 範囲外の介入倍率を、★捨てない', () => {
  ...
  it('🔴 ★エンジンは、★範囲外を渡すと**受け取った値と適用した値**を残す', () => {
     ...
     *   → ★**介入は、★エンジンにだけ在ります**（★画面にも API にも口がありません ＝ ★**未実装**）。
     *     ★だから `intervention_mult` も `intervention_log` も空のままです。
     *   ⚠️ ★**繋ぎ忘れではありません。★機能がまだ作られていない**、という状態です。
  ...
  it('🔴 ★包みが、★介入をエンジンへ渡していない（★いまの姿を固定する）', () => {
    /**
     * 🔴 ★**これは要件ではありません。★「いまの状態を留める」検査です**（★`CK-11`）。
     *
     *   ⚠️ ★**「介入を渡してはいけない」とは言っていません。** ★逆です —
     *     ★**渡し始めた日に、★関係する項目を見直せるように**落とします。
     *   ✔ ★いま渡していない理由（★2026-09-20 に確かめました）:
     *     ★`apps/web/src` に `intervention` は **0 件**。★API の口は `healthz` と `rig-lab` だけ。
     *     → ★★**介入を受け付ける口がどこにも無く、★渡すものが存在しません。**
     *     → ★**未実装であって、★繋ぎ忘れではありません。**
     *   ★落ちたら見直す先: ★`F2-LOG-NOT-PUBLISHED`（★介入ログの公開）と、★この検査そのもの。
     */
