 ...
 * ⚠️ ★**2026-09-18・AL-6 で測り直しました**（`REPORT_AL6_ODDS_COST_20260918.md`）。
 *    ★本番機への換算で **1 レース 70〜98 秒**（★旧: 138 秒 ＝ D-071 前のエンジンの値・失効）。
 *    ★**1 日 240R（D-007 再改訂・6 分）で 4.7〜6.5 時間 ＝ 使用率 19〜27%**。
 *    ⚠️ ★換算は換算です。★**最終確認は配備後の周の実測**（R-28）。
 *    ★モンテカルロは DB トランザクションの外で走ります。
 */
