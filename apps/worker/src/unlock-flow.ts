 ...
 *
 * 【🔴 ★どの環境の値か（★2026-09-26 に書き足しました）】
 *   ★★**staging を 2026-08-12 に書き出したスナップショット**の分布です。★**production ではありません。**
 *   ✔ ★母集団: ★`docs/pool-staging.json`（★7.0MB・★2026-08-12 の書き出し）
 *   ✔ ★測った実行: ★`npm run verify:race --pool docs/pool-staging.json --real-ability --b6-wired`
 *     （★`docs/ACCEPTANCE_P3.md:31`）
 *
 *   ⚠️ 🔴 ★**D-053 の本文は 1 度「本番の実分布」と書いており、★2026-09-20 に訂正されています。**
 *      ★その訂正は ★正典の ★**長い 1 行の末尾**にあります。
 *      🔴 ★2026-09-26、★開発側（私）が ★先頭だけ読んで ★**「本番の分布」と受け取りました**。
 *        ★そのままオーナーに ★「環境が違う可能性があります」と ★不確かな言い方で報告しました。
 *      → ★★**ここに書いておけば、★次に読む人は正典の長い行を読まずに済みます**（★D-052 の形）。
 *
 *   ⚠️ ★だから ★`unlockDrift()` の比較は ★**本番の生きた分布 ↔ staging の 8/12 スナップショット**です。
 *      ★★**同じ土俵ではありません。** ★警告は「動く入力の上にゲートが立っている」を知らせる仕組みとして
 *      ★正しく働きますが、★**pt 差をそのまま「悪化」と読まないこと**。
 *   ✔ ★実測（★2026-09-26・本番）: ★引退 5,543 頭・★ゲーム内週 297・★6 時間で 29 頭 誕生。
 *     ★8/12 の集団とは ★**別物**です。★若い馬が絶えず入るので ★下の裾（p10）が伸びます
 *     （★`p50` 73.2% と `p90` 87.3% は ★4 日 不動でした）。
 */
