...
/** Shared presentation for live odds and the explicitly labelled development preview. */
export function OddsBoard({ race, entries, odds, type, oddsPath }: {
  race: OddsRow; entries: OddsRow[]; odds: OddsRow[]; type?: string | undefined; oddsPath: string;
}) {
  ...
  const Guide = (): React.ReactElement => (
    <div className="a-panel" style={{ width: 330, flex: '0 0 330px' }}>
      ...
      <div style={{ padding: '12px 14px 14px', display: 'flex', flexDirection: 'column', gap: 9, fontSize: 13, fontWeight: 900, color: 'var(--a-ink-2)', lineHeight: 1.65 }}>
        <span><span style={{ color: 'var(--a-num-rank)' }}>赤の数字</span>は人気 1〜3 位。<span style={{ color: 'var(--a-num-time)' }}>緑の数字</span>はそれ以外</span>
  ...
  return (
    <div style={{ padding: '22px 0 40px' }}>
      ...
      <div className="rc-tabs" style={{ display: 'flex', alignItems: 'flex-end', gap: 6, marginTop: 14, flexWrap: 'wrap' }}>
        {visibleTabs.map((t) => (
          <TabButton key={t.key} label={t.label} selected={t.key === tab.key} href={`${oddsPath}?type=${t.key}`} />
        ))}
      </div>
        ...
        <>
          ...
          <div className="a-panel strong" style={{ borderRadius: '0 10px 10px 10px', marginTop: -2 }}>
            ...
            {win.map((o, i) => {
              ...
              return (
                <div key={gate} className="story-odds-row" style={{
                  ...
                  <span className="story-odds-rank" style={{ width: 52, flex: '0 0 52px', display: 'flex', alignItems: 'baseline', gap: 3 }}>
                  ...
                  <span className="story-odds-gate" style={{ width: 44, flex: '0 0 44px', display: 'flex', justifyContent: 'center' }}><FrameBadge gate={gate} fieldSize={fieldSize} w={38} h={28} font={19} /></span>
                  <span className="story-odds-name" style={{ flex: '0 0 196px', display: 'flex', alignItems: 'center', gap: 8, fontSize: 17, fontWeight: 900, color: 'var(--a-ink)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {nameOf.get(gate) ?? `${gate}番`}
                  </span>
                  <span className="story-odds-support" style={{ flex: 1, minWidth: 120, height: 16, borderRadius: 8, background: 'var(--a-panel-2)', border: '2px solid var(--a-edge-soft)', overflow: 'hidden' }}>
                    <span style={{ display: 'block', height: '100%', width: `${w}%`, backgroundImage: top3 ? 'var(--a-gloss-red)' : 'var(--a-gloss-blue)' }} />
                  </span>
                  <span className="a-num story-odds-win" data-label="単勝" style={{ width: 88, flex: '0 0 88px', textAlign: 'right', fontSize: top3 ? 32 : 26, color: top3 ? 'var(--a-num-rank)' : 'var(--a-num-time)' }}>
                    {formatOdds(Number(o['odds']), Boolean(o['capped']))}
                  </span>
                  <span className="a-num story-odds-place" data-label="複勝" style={{ width: 126, flex: '0 0 126px', textAlign: 'right', fontSize: 18, color: 'var(--a-ink-2)' }}>
            ...
            <div className="a-panel strong" style={{ flex: 1, minWidth: 0 }}>
              <div className="a-band" style={{ height: 38, padding: '0 16px', justifyContent: 'space-between' }}>
                ...
                <span style={{ display: 'flex', alignItems: 'center', gap: 14, fontSize: 12, fontWeight: 900 }}>
                  ...
                  <a className="rc-exch" href={`${oddsPath}?type=quinella`} style={{ color: '#fff' }}>すべて表示 →</a>
                </span>
              </div>
