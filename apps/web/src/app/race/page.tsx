 ...
 *   ★あわせて `isDeformedHorseAsset` の書き忘れを直したので、★型 B の素材の色が変わりました
 *   （★デフォルメの絵に写真寄りの表が掛かっていた）。
 *   ⚠️ ★**画面の見た目は変わりません** — ★`HORSE_TYPE_BY_GATE` は全枠 型 A で、
 *      ★型 B の素材は 1 度も読まれないためです（★型 B を戻す日に効きます）。
 *   ★目録も中身も変わるので必ず上げること。
 */
/**
 * ★73: ★焼き方を ★**非可逆 q90** にした（★オーナー決定 2026-09-24・★2026-09-02 の「可逆」を置き換え）。
...
/**
 * ★**4 角の撮り方**（★2026-09-11・★オーナー ④「コーナーを出来るレベルで一瞬見せる」）。
 *
 *   ★`?corner=far`（★**既定**）… ★うんと引いた俯瞰（★参考映像の大きさに寄せた）
 *   ★`?corner=wide`            … 俯瞰ワイド（★従来の代用カメラ）
 *   ★`?corner=front`           … ★従来の正面固定（★奥から迫る）★**切り戻しの道**
 *
 * 【★なぜ既定を替えたか】★オーナー ④「★コーナーは今のところ 1 つも上手くいっていません。
 *   ★コーナーを ★**出来るレベル**で作ってください」。
 *   ★不合格の中身は ★**走り方**なので、★**馬を小さくすれば消えます**（★実測 19.9% → 7.7%）。
...
/**
 * ★斜め前（`diag-front-v4`）: 騎手は上中央、鞍布は騎手の脚の下に少し見える。ゼッケンは描かない
 *
 * ⚠️ ★**鞍布の窓を右へ広げないこと**（★2026-09-12・オーナー指摘「馬の目が赤色になる」）。
 *
 * 【★何が起きていたか — ★塗る画素を可視化して分かりました】
 *   ★鞍布の窓は `nx 0.05〜0.95` ＝ ★**絵の幅ほぼ全部**でした。★この窓の中の
 *   ★「無彩色で明るい画素」を ★**枠色**で塗るので、★馬の ★**白目**が枠色になります。
 *   ★オーナーの自馬は 3 番＝★枠色は ★**赤**。★＝「目が赤色になる」。
 *   ★実測（`pose01`）: ★窓の中で塗られる画素 ★**3023 個**、★nx 0.06〜0.94。
...
export default function RacePage(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const boot = async (): Promise<void> => {
      ...
      const neededAssets = broadcastV2ScriptAssets(
        ...
        /**
         * ⚠️ ★`?corner=far` も ★**引きの素材（`high-diag-v2`）が要ります**。
         *    ★これを足し忘れると、★4 角だけ ★**素材が無い**まま描かれます。
         */
        !FOURTH_CORNER_FRONT_WEB || CORNER_STYLE_WEB !== 'front',
      );
  ...
  const render = useCallback((d: number) => {
     ...
     */
    /**
     * ★**出番が後の音源は、★描き始めてから取りに行きます**（★2026-09-24）。
     *   ★`gallop`（0.61MB・32.4 秒から）と `crowd`（1.54MB・ゴール後）で ★**2.15MB**。
     *   ★作った時点で全部取りに行くと、★**音を出さない人にも**その 2.15MB が落ちます。
     * ⚠️ ★ここ（★描く輪）から呼びます。★「始めた時に 1 回」だと、
     *    ★自動再生しない経路（`?entry=1` / `?dev=1`）で呼ばれません。★何度呼んでも構いません。
     */
    ...
    if (renderer === 'v2') {
       ...
       */
      /**
       * ★このカットが始まった切り替え（★台本の切り替え表から引く）。
       * ⚠️ ★`reduce` の初期値を `d` にすると ★**常に `d` が返り、経過が 0 になります**
       *    （★2026-09-10 に実際にやりました。★登場アニメが透明のままで図が出ませんでした）。
       */
      type ShotChange = { readonly displaySec: number; readonly from: BroadcastV2ShotId; readonly to: BroadcastV2ShotId };
      const cutChange = (motionTimeline ?? built).shotChanges
      ...
      if (startCutInActive) {
        ...
        /**
         * ⚠️ ★**「スタートしました！」を画面中央に大書きしません**（★2026-09-11・オーナー指摘）。
         *    ★「これは実況中継の枠内で出すものです」。
