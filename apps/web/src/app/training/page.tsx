...
export default function TrainingPage(): React.ReactElement {
   ...
   *
   * ✅ ★**指示は繋がっています**（★移行 `0057`〜`0059`・★このファイルの `:195` 以下）。
   *
   * ⚠️ 🔴 ★**2026-09-26 まで、ここに ★「繋いでいません。保存先が在りません」と書いてありました。**
   *    ★それは ★2026-09-20 の時点では正しく、★`0057` で塞がれた後も ★**残っていました**。
   *    ★同じファイルの 200 行下に ★「指示をサーバーへ送ります（`0057` の `set_training_order`）」
   *    ★と書いてあるのに、★**矛盾する註記が同居**していました。
   *    🔴 ★実害: ★2026-09-26、★開発側（私）が ★上だけ読んで ★**オーナーに
   *      ★「遊びの中心が繋がっていない。判断が要る」と誤って報告しました**。
   *    → ★★**直した便で、直した所の古い註記を消す。** ★註記は ★**現物より長生きします**。
   *
   * 【★いまの実物】
   *   ★① ★置き場所 … ★`training_orders`（★`0057`。★`(horse_id, week)` が鍵・`0059`）
   *   ★② ★画面が送る … ★`set_training_order`（★下の `:195` 以下）
   *   ★③ ★ワーカーが読む … ★`training-runner.ts:355` が ★`defaultMenu` の ★**前に**読み、
   *     ★`const ordered = orderOf.get(...) ?? null;` → ★`ordered ?? defaultMenu(...)`。
   *     ★★指示が在れば ★それを使い、★無ければ ★**自動で決まります**。
   */
