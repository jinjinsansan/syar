 ...
 *   ⚠️ ★私はこの切り出しで ★**一度 残しました**（★置換が改行コードで外れた）。
 *     ★`npm run typecheck` は ★**通りました**。★`next build` が捕まえました。
...
/** ★状態を出す小さな枠（★空を「無い」と言い切らないため） */
function Note({ children }: { readonly children: React.ReactNode }): React.ReactElement {
  return <p style={{ padding: '24px 40px', fontWeight: 900, color: 'var(--a-ink-2)' }}>{children}</p>;
}
...
export function HorseDetailView({ horseId }: { readonly horseId: string }): React.ReactElement {
  ...
  if (state === 'loading') return <Note>読み込んでいます…</Note>;
  if (state === 'login') return <Note>この馬を見るには、<a href="/login">ログイン</a>してください。</Note>;
  if (state === 'setup') return <Note>牧場の初回設定が必要です。<a href="/setup">設定へ</a></Note>;
  if (state === 'missing') return <Note>この馬は見つかりませんでした。</Note>;
  if (h === null) return <Note>{error ?? '読み込めませんでした。'}</Note>;
  ...
  return (
    <>
      ...
      */}
      <div className="show-narrow"><HorseResume horse={h} {...(discovery === null ? {} : { discovery })} /></div>
