...
/**
 * ⚠️ ★**ここは以前の TOP です**（★2026-09-13 に `/` から `/lp-arcade` へ退避）。
 *    ★`/` は「馬物語」の LP（`app/lp-preview/story.tsx`）に入れ替わりました。
 *    ★消していないのは、★こちらだけが ★**次のレース・最新の確定レースをサーバーから読んで**
 *    ★導線に出しているからです（★下の `readClient`）。★新しい LP にその口はありません。
 */
/**
 * ★TOP（未ログインの LP）— 正本 design/hud-ds/components/landing［アーケード］
 *   初めて来た人が「何のゲームか」「何をすればいいか」を 1 枚で理解し、「無料ではじめる」を押す。
 *   ログイン後の着地は `/stable`（牧場ホーム。レビュー裁定 Q-WEB-03: 着地は「何のゲームか」の宣言）。番組表は `/races`。
 *   Auth 導入後はログイン済みなら `/` から `/stable` へ送る。
