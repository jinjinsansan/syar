 ...
 *
 *   🔴 ★**2026-09-24 に絵が 1 種になりました**。★この 3 値が指すのは ★**言葉**です
 *      （`/train` の `FACE_WORD`）。★顔の絵は平常の 1 枚だけです。
 *      ★理由: ★平常の頭部から「上機嫌」「疲れ」を画像編集で作らせたところ、
 *      ★2 回とも頭の形と線が引き直されて ★**別キャラ**になりました（★空き `LOOK-FACE-MOODS-MISSING`）。
 *      ⚠️ ★旧素材 `train-face-*.webp` は ★**レースの馬と絵柄が違う**ので使いません（★オーナー差し戻し 3 回）。
 *   ★段を増やさないこと。
