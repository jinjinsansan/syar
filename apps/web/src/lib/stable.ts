...
/**
 * ★**調教画面の気分（3 段）を選ぶ**（★2026-09-23）
 *
 *   ★**2026-09-24 に作り方が変わりました**（★オーナー決定 D-4）。★頭部の絵は ★**1 枚**で、
 *      ★表情は ★**まぶた／眉だけの部品を重ねて**作ります（`/train` の `FACE_PART` と `FACE_WORD`）。
 *      ★理由: ★表情ごとに頭部を焼き直させたところ、★2 回とも頭の形と線が引き直されて
 *      ★**別キャラ**になりました（★オーナー差し戻し 3 回と同じ形）。
 *      ⚠️ ★旧素材 `train-face-*.webp` は ★**レースの馬と絵柄が違う**ので使いません。
 *   ★段を増やさないこと（★増やすなら部品も要ります）。
