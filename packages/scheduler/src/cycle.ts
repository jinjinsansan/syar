...
/**
 * ★1 サイクル = **3 分**（★**D-007 改訂**・2026-09-18・T-13・§10.2）
 *
 * ★**旧**: 10 分（1 日 144R）。★**目的はレース密度を 3.3 倍にすること**で、
 ...
 *
 * ★**2026-09-18・D-007 改訂 ① で作り直しました**（★旧: 0 / 0:30 / 3:00 / 9:30 / 10:00）。
 *   ★**発売は 2 分**（`salesOpen` 0:30 → `salesClose` 2:30）。
 *   ★生成は `LOOKAHEAD_RACES = 2` のままなので **6 分の余裕**があります（★本数は変えません）。
 */
export const PHASE_OFFSET_MS = {
  ...
  salesOpen: 30 * 1000,
  /** 発売締切 */
  salesClose: 2 * 60 * 1000 + 30 * 1000,
  /** パドック・返し馬演出（発走まで） */
  parade: 2 * 60 * 1000 + 30 * 1000,
  /** 発走 */
  start: CYCLE_MS,
} as const;
 ...
 *
 * 【なぜ60分か】
 *   ★**20 サイクル**（★2026-09-18・D-007 改訂 ⑤・AL-10。★旧: 6 サイクル＝10 分 × 6）。
