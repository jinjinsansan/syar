...
/**
 * ★**1 周のうち、★配合の追いつきに使ってよい割合**（★簿 `BREEDING-ONLY-ONE-WEEK-PER-CYCLE`・2026-09-21）
 *
 * 【🔴 ★なぜ「1 周に N 週」と決めないのか（★レビュー側の裁定）】
 *   ★1 週の費用は ★**一定ではありません**。★実測（★staging・2026-09-21）:
 *   ```
 *   20 頭 … 19.2s   14 頭 … 15.7s   13 頭 … 13.5s   年の変わり目 … 12.6s
 *   ```
 *   → ★「N 週」で決めると、★**多産週が続いた日に予算を超えます**。
 *   ⚠️ ★そして ★**VPS からの往復は測れていません**（★鍵がありません）。★費用は環境で変わります。
 *   → ★★**時間で切ります。★1 週 進めるごとに経過を見て、★超えたらそこで止め、★次の周へ持ち越す。**
 *     ★★費用が変わっても、★環境が変わっても、★予算は守られます（★`AL-9`「数を導く」と同じ作法）。
 *
 * 【★なぜ 0.15 か — ★下限が導けます】
 *   ★実時間では ★**40 周で 1 週**しか進みません（★1 週 ＝ 4 時間 ＝ `CYCLES_PER_WEEK`）。
 *   → ★★**1 周に 2 週 進めれば、★必ず追いつきます**（★78 週の遅れが ★39 周 ＝ 約 4 時間で解消）。
 *   → ★★**予算は「最悪の週を 2 回 通せる」以上**でなければ、★永久に追いつきません。
 *   ```
 *   最悪の週（実測）19.2s × 2 = 38.4s
 *   1 周 = 360s  →  38.4 / 360 = 10.7%
 *   ```
 *   🔴 ★**0.10 では足りません**（★36s < 38.4s）。★余裕を見て **0.15**（★54s ＝ 最悪 2.8 週ぶん）。
 *
 * ⚠️ 🔴 ★**この 0.15 は「いまの実測」に基づく見込みです。**
 *    ★足りているかどうかは ★**予算ではなく、★遅れが縮んでいるか**で判定します
 *    （★`main.ts` が ★**N 周 続けて縮まなければ投げます**）。
 *    ★★見込みが外れても、★黙って遅れ続けることはありません。
 */
export const BREEDING_BUDGET_RATIO = 0.15;

/**
 * ★1 周のうち、★配合の追いつきに使ってよい時間（ms）。
 * ⚠️ ★**直書きしません** — ★`CYCLE_MS` が動いた日に自動で追随します（★D-052）。
 */
export const BREEDING_BUDGET_MS = CYCLE_MS * BREEDING_BUDGET_RATIO;
