...
/**
 * ★**候補が丸ごと枯れたことを知るための線**（★D-102 ⑤・D-079 ⑧）。★**MK-2 で言い直しました**（2026-09-19）。
 *
 * 【🔴 ★これは「棚が埋まるか」の線では**ありません**】
 *   ★旧の註記は「NPC の現役プールの在庫の下限」でした。★それだと ★**MK-1 が数えていた 7,333 頭**
 *   （★現役 NPC 全体）に対する線に読めます。★実際に買えるのは ★**走った実績のある馬だけ**です。
 *   ✔ ★測りました（2026-09-19・staging）: ★**候補 732 頭**。★200 は ★**その 27.3%**。
 *
 * 【🔴 ★棚が埋まるかは、この線では分かりません】
 *   ✔ ★同じ日の実測: ★帯 0 = 656 頭 / 帯 1 = 56 / 帯 2 = 18 / ★**帯 3 = 2（3 口に足りない）** / ★**帯 4 = 0**。
 *   → ★候補は 732 頭（★下限の 3.7 倍）あるのに、★**棚は 5 段のうち 2 段が空**です。
 *   → ★★**総数の下限では、いま起きている欠品を検出できません。**
 *   ★そちらは `planListings` の **`shortfall`**（★T11-1 ②）が帯ごとに数え、★`onAlert` を鳴らします。
 *   ★**役割が違います** — ★ここは「候補そのものが消えた」を見る線です（★MK-1 のような集合の取り違え・
 *   ★プリシードの失敗・★`race_entries` の欠落）。
 *
 * 【★上の帯が空なのは不具合ではありません（★入力を先に見た）】
 *   ★価格は `npcStudFee` ＝ `3,000 + G1勝利数 × 8,000 + 総獲得PP / 20`。
 *   ✔ ★staging の実測: ★着順の付いた出走 **875 件**・★重賞は **G3 が 2 件で G1 は 0 件**。
 *   → ★`g1_wins > 0` が **0 頭**なのは ★**正しい**（★G1 が 1 度も行われていない）。
 *   → ★帯 4（7,600 EP）に届くには 総獲得 **92,000 PP** が要ります。★世界がまだ若いだけです。
 *   ⚠️ ★`prize_pp` は **875 件すべてに入って**います（★PR-1・323 件が 0 より大）。★入力は繋がっています。
 *
 * ⚠️ ★**200 という数そのものに導出はありません。** ★「候補の 1/4 を切ったら異常」という当座の線です。
 *    ★候補の実数（★今 732）が大きく動いたら、★**割合として読み直してください**。
 */
 ...
 * ⚠️ ★**5 段のまま**にしています（★口数 5 × 3 ＝ 15 口は T-10 から変えていません）。
 *    ★段を増やすと口数が増え、★D-102 ③「選び直しが成立しない」に近づきます。
 *
 * 【✔ ★**測って決めました**（★2026-09-19・`apps/cli/src/market-price-distribution.ts`）】
 *   🔴 ★最初は丸い数 `[3,000 / 4,000 / 6,000 / 10,000 / 20,000]` を仮に置き、
 *      ★staging の実測（★候補 732 頭）を見て「★測っても決まらない」と報告しました。
 *      ★**それは誤りでした** — ★staging の 875 走は ★**1 頭のキャリアの 1.04%**（★6.5 時間ぶん）で、
 *      ★**まだ測れていなかった**だけです（★裁定 `REVIEW_T11_BANDS_VERDICT_20260919.md`）。
 *
 *   ★定常の集団（★3,000 頭 × 番組表 26 日 ＝ キャリア 1 本分）をシミュレータで走らせ、
 *   ★価格の分位点 **50 / 75 / 90 / 99%** で切りました。★シード 42 / 7 / 1234 で
 *   ★**どの帯も 3 口 埋まります**（★最上段で 28〜30 頭）。
 *
 *   ⚠️ ★**シミュレータで測った値です**（★本番の実測ではありません・PO-5）。
 *      ★入っていないもの: ★育成（調教・調子・疲労）／★引退と世代交代／★NPC の配合。
 *      ★staging のワーカーを動かせたら、★裏を取ってください。
 */
export const PRICE_TIERS_EP: readonly number[] = [3_000, 3_200, 3_600, 4_200, 7_600];
