 ...
 *
 * 【★サイクルとの関係を式で書く】
 *   ★1週 = 4時間 = **3分サイクル 80本ちょうど**です（★D-007 改訂・2026-09-18。★旧: 10分 × 24本）。
...
/**
 * ★1 ゲーム内週の実時間（正典 **D-007**: リアル 4 時間）。
 * ★**サイクルの長さを変えてもここは動きません** — ★D-007 改訂の目的は
 *   ★「レース密度を 3.3 倍にする」ことで、★**積み上げる実時間は変えない**ことでした。
 */
const WEEK_REAL_MS = 4 * 60 * 60 * 1000;

/**
 * ★1 週あたりのサイクル数 ＝ **4 時間 ÷ `CYCLE_MS`**（★3 分なら **80**・10 分なら 24）。
 *
 * ★**直書きをやめました**（★2026-09-18・D-007 改訂・AL-9 と同じ扱い）。
 *   ★このファイルの冒頭が「数値を別々に書くと、片方だけ動かしたときに静かにずれる」と
 *   ★**書きながら `= 24` の直書きだった**ので、★**週境界とサイクル境界の一致を構造で保ちます**。
 * ⚠️ ★割り切れないサイクル長なら**投げます**（R-27）。★端数のまま進むと週境界がサイクルの途中に来ます。
 */
export const CYCLES_PER_WEEK = ((): number => {
  const n = WEEK_REAL_MS / CYCLE_MS;
  if (!Number.isInteger(n) || n <= 0) {
    throw new Error(`CYCLES_PER_WEEK: 1 ゲーム内週（4 時間）が CYCLE_MS で割り切れません（CYCLE_MS=${CYCLE_MS}・n=${n}）`);
  }
  return n;
})();
