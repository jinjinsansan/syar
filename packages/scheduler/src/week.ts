...
/**
 * ★**キャリア上限**（正典 §7.1）。★**2026-09-19・CC-1 ③ で 24 → 40**。
 *
 * 【★なぜ動かしたか — ★上級条件が半分 成立していませんでした】
 *   ✔ ★実測（**PO-6/CC-4**・`market-price-distribution.ts`）:
 *     ★上級条件（open・重賞）は ★**資格者不足で 4〜5 割が中止**になっていました。
 *   ✔ ★**プールの大きさは効きません** — ★「4 勝以上」を同時に持つ頭数は
 *     ★**プール 3,000 でも 7,333 でも 7.7 頭**（★±0・**PO-7**）。
 *   → ★★**効く梃子はキャリア長でした**: ★24 → 40 で ★**成立率 86.2% → 99.1%**、
 *     ★上級条件の開催が **2,121 → 3,548 本**。
 *
 * ⚠️ ★**これは較正定数ではなく、正典 §7.1 の写し**です。★オーナー決定（2026-09-19）。
 * 🔴 ★**写しをもう 1 か所に作らないこと**（D-052）。
 *    ✔ ★2026-09-19 に `economy-balance.ts` が `starts: 24` と**直書き**していたのを見つけ、
 *      ★ここを引く形に直しました。
 * ⚠️ ★**取り直しが要ります**: ★V-4/V-5/V-6（★D が基準・AL-1 付き）／★§3.4 の収支／★V-14。
 */
export const CAREER_RACE_LIMIT = 40;
