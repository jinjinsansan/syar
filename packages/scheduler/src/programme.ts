/**
 * 番組表（正典 §10.3）— ★1日240R の内訳（★D-007 再改訂・2026-09-18。★144R → 480R〔撤回〕→ ★240R）
 *
...
/**
 * ★**`world_state.day_started_at` を「古い」と呼ぶ線** [時間]（★2026-09-25・裁定 §7 ①）。
 *
 * 【★なぜ要るか】
 *   ★`0080` のデイリー EP は 1 日 1 回の鍵を ★この行から作ります。
 *   → ★ワーカーが止まると鍵が進まず、★**誰も受け取れません**（★向きは安全側）。
 *   🔴 ★ただし ★画面が「受け取り済み」と出し続けるのは誤りです（★配布が止まっているだけ）。
 *
 * 【★28 の根拠】
 *   ★正常時 `day_started_at` は ★**0〜24 時間前**（★`DAY_MS`）。★24 時間を超えたら 1 回 進みそこねている。
 *   ★ワーカーは 6 分ごとに書くので境目直後は 24 時間 + 数分に届きえる → ★余裕 4 時間で ★**28**。
 *
 * ⚠️ ★正は ★**SQL の `day_boundary_stale_after_hours()`（`0081`）**で、★ここはその写しです。
 *    ★`apps/cli/test/ep-grant-sql.test.ts` が突き合わせます（★ずれたら落ちる）。
 */
export const DAY_BOUNDARY_STALE_AFTER_HOURS = 28;
 ...
 *   ★10 分なら 144、★6 分なら 240 が**自動で出ます**。
 *
 * ✅ ★**`RACES_BY_CLASS` は 2026-09-18 の D-007 再改訂で合計 240 に振り直しました**（★正典 §10.3 の表。★比は 10 分の世界のまま）。
...
/**
 * ★**1 枠の分数**（★サイクルから導く）。★6 分なら 6・10 分なら 10。
 */
 ...
 *   🔴 ★**3 分にすると 480 枠になり、枠 54 は 09:00 ではなく 02:42 になります**（★6 分でも 05:24 とずれます）。
...
/**
 * その日のうち何番目の枠か（0〜`RACES_PER_DAY`−1。★6 分なら 0〜239）。
