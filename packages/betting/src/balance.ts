 ...
 *
 * 【★これは「打ち切りを無視できる水準」ではありませんでした】
 *   当初は `c ≧ 1` の打ち切り対策と考えていましたが、λ*=30 では
 *   `P(c=0) = e⁻³⁰ ≒ 9×10⁻¹⁴` で、打ち切りは**説明になりません**。
 *
 *   実際に残っている誤差は**発売判定そのものが生む選択バイアス**です:
 *     売るか売らないかを **`p̂ ≧ p_min`（＝ c ≧ λ*）で決める**ので、
 *     真の p が p_min 近傍の目は「p̂ が上振れしたときだけ売られます」。
 *     売られた目は p̂ が過大 → オッズが過小 → 払戻が不足します。
 *
 * 【★λ⁻¹ で縮む（導出と計算が一致）】
 *     境界の幅（p 空間の相対幅）          ～ 1/√λ*
 ...
 *
 *   二項分布の厳密計算（20レース）も同じ:
 *     λ*=15 → −0.71pt   λ*=30 → −0.37pt   λ*=60 → −0.19pt   （比はどちらも約2）
 *
