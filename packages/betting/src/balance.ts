...
/**
 * ★発売下限 p_min まわりの設計余裕 M·p_min（正典 D-035）。
 *
 * 【★λ* を上げても残差は縮みませんでした — 実測で確認済み】
 *   一時 60 にしましたが、**30 に戻しました**。経緯を残します。
 *
 *   当初は `c ≧ 1` の打ち切り対策と考えましたが、λ*=30 では
 *   `P(c=0) = e⁻³⁰ ≒ 9×10⁻¹⁴` で、**打ち切りは残差の説明になりません**。
 *   ★にもかかわらず「λ⁻¹ で縮むから 60 にすれば半減する」と予測しました。
 *     **打ち切りが説明にならないと分かっていたのに、λ⁻¹ で縮む項（＝打ち切り）で
 *     予測した**ので、根拠が最初から成り立っていませんでした。
 *
 *   同じ100レース・同じ着順（`deriveRng(SEED, S.FINAL, i)` が同一）で比較した実測:
 *
 *     券種        λ*=30    λ*=60      差
 *     三連単     −0.48    −0.38    +0.10   （予測は +0.24）
 *     三連複     −0.27    −0.26    +0.01   （予測は +0.14）
 *     他5券種    ほぼ同一          0.00〜0.04
 *
 *   ★**M を2倍にして得たのは 0.10pt。本番の1レースは 116秒 → 256秒**でした。
 *
 * 【★判定の余裕を作るのは本番の M ではなく測定のレース数】
 *     余裕 = 1.00pt − ( |残差| + 2×SE )
 *   このうち **2×SE は測定の不確かさであって実体ではありません**。
 *   レース数を 100 → 400 にすれば SE は半分になり、**本番の計算量を増やさずに**
 *   余裕が広がります。「余裕ゼロの解は採らない」は残差を消せという意味ではなく、
 *   **判定が測定精度に食われるな**という意味でした。
 *
 * ⚠️ これは較正定数です。必要な試行数が**そのまま比例**して増え、
 *    レース生成の所要時間に直結します（本番機の実測: λ*=30 で1レース116秒）。
 * ⚠️ **1行で書くこと**（MARGIN 参照）。
 */
// prettier-ignore
