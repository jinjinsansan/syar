...
/**
 * 数値形質のキー一覧。アレル継承・隔世遺伝・突然変異はこの12形質に対して回る。
 * ⚠️ ★**「11形質」と書いてありました**（★2026-08-31 差し戻し）。★`heavy_aptitude`（D-015）が
 *    ★足されたときにコメントが追随していません。★機械は `NUMERIC_TRAITS` から差分で導くので
 *    ★追随しますが、★**人が読む側だけが黙って古くなります**（R-22 の家族）。
 *    → ★**形質を足すときは、この行も一緒に直すこと。**
