 ...
 *
 * 【★正典に無い値（＝較正定数。照会に上げます）】
 *   ⚠️ 🔴 ★**2026-09-19 まで「正典が数値で与えているのは `BASE_GAIN = 12`」と書いていました。**
 *      ★正典 §13.1 は ★**7.8**（★D-045 / D-047 の較正に追随・レビュー側が `7a577a2` で直しました）。
 *      → ★**この註記のほうが古かった**のであって、★下の実装（7.8）は正しいままです。
 *   ★正典が数値で与えているのは `BASE_GAIN`（★下の 7.8 と同じ） / headroom 指数 `0.7` /
...
/**
 * 成長式の基準量（正典 §7.3 は「**既定**」と書いており固定値ではない・D-048）。
 *
 * 🔴 ★**ここが生きた台帳です**（★2026-09-19・AUDIT-CANON-BASE-GAIN）。
 *   ★正典 §13.1 も **7.8** になりました。★写しを作らないこと —
 *   ★`apps/cli/src/diag-growth-scale.ts` が 12 を直書きして「★正典の式」と名乗っており、
 *   ★**道具が嘘をついていました**（★R-30）。★いまは ★**ここから import**しています。
 *
