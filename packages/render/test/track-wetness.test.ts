...
describe('★馬場状態で走路が暗くなる', () => {
  ...
  it('★悪くなるほど濃い（単調）', () => {
    ...
    /**
     * ★**濃くしすぎない**（走路が黒い帯にならないこと）。
     *
     * ⚠️ 【★2026-08-30・この線を 0.5 → 0.7 へ動かしました。理由を残します】
     *    ★私は最初 良 0 / 稍重 0.12 / 重 0.24 / 不良 0.34 と置き、
     *    ★その周りに緩く 0.5 の線を引きました。
     *    ★ところが ★**オーナー評は「何も変わっていません」**でした（実測 重 87.4%）。
     *    → ★較正を 0.22 / 0.42 / 0.55 へ上げたところ、★**この線に当たりました。**
     *
     * ⚠️ ★**落ちたから緩める、は最も危険な形**です。★意図に照らして判断しました:
     *    ・★この線の**意図**は「走路が**黒い帯**にならないこと」
     *    ・★0.55 でも**明るさは 60% 残り、模様も全部残ります**（黒い帯ではない）
     *    ・★0.5 という数は ★**オーナーが却下した較正**の周りに引いた囲いでした
     *    → ★意図は保ったまま、囲いを引き直します。★0.7 を超えたら模様が消えるので、そこが線です。
     */
    expect(bad, '★不良が濃すぎる。走路が黒い帯になって模様が消える').toBeLessThan(0.7);
  });
  ...
  it('★稍重と重が、目で分かる差になっている（★「作った」と言える量か）', () => {
     ...
     */
    /**
     * ⚠️ ★**この下限は 2 度目です。** 最初 0.10 で通りましたが、
     *    ★実画面で 重 87.4% にしかならず、★オーナー評「何も変わっていません」。
     *    → ★**「検査が通った」は「見える」を意味しません。**
     *    ★下限を、実画面で確かめた値（稍重 0.22 / 重 0.42）に合わせて引き直します。
     */
    expect(trackWetnessAlpha('yielding')).toBeGreaterThanOrEqual(0.20);
    expect(trackWetnessAlpha('soft') - trackWetnessAlpha('yielding')).toBeGreaterThanOrEqual(0.15);
  });
