...
describe('台本 v6 — 直線を 4 カットに割る', () => {
  ...
  it('★俯瞰のカットは、真横の半分以下にしか描かない', () => {
     ...
     */
    const cases = [
      { leadS: 1170, shot: 'fourth-corner-far', style: 'far' as const },
    ];
  ...
  it('★4 角の撮り方は選べる（★既定は正面固定・★俯瞰へ 1 手で戻せる）', () => {
    /** ⚠️ ★**1170m が 4 角**です（★2026-09-12 に走路の本当のコーナーへ貼り直した） */
    expect(frameAt(1170, 'v6').shot, '★既定は画面と同じ front').toBe('fourth-corner-front');
    expect(frameAt(1170, 'v6', 1, 'front').shot, '★明示しても同じ').toBe('fourth-corner-front');
    expect(frameAt(1170, 'v6', 1, 'wide').shot).toBe('fourth-corner-wide');
    expect(frameAt(1170, 'v6', 1, 'far').shot).toBe('fourth-corner-far');
    expect(frameAt(1170, 'v6', 1, 'far').top4HeightRatio)
      .toBeLessThan(frameAt(1170, 'v6', 1, 'wide').top4HeightRatio);
  });
...
describe('★走路の接線の角度（★絵を回すときに使う）', () => {
  ...
  it('★コーナーでは傾いている（★だから絵を回す意味がある）', () => {
    expect(deg(angleAt(1170)), '★4 角').toBeGreaterThan(10);
  });
