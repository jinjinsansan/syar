...
describe('既定台本（2026-08-28 から v6）', () => {
  ...
  it('⑥ 第4コーナーは正面から（俯瞰は撤回・外すのも撤回）', () => {
     ...
     */
    /**
     * ⚠️ ★**進行の割合ではもう当たりません**（★2026-09-12）。
     *    ★コーナーのカットは走路の本当のコーナーへ貼るようになったので、
     *    ★桜星賞の 4 角は 進行 **0.675〜0.750**（★1080〜1200m）です。
     */
    for (const r of [0.69, 0.71, 0.74]) {
      expect(shotAt(r, scriptOf('')), `進行 ${r}`).toBe('fourth-corner-front');
    }
    /**
     * ⚠️ ★**旧台本（v4 / v5）は割合のままです**（★切り戻しの道を動かさない）。
     *    ★だから進行の割合が v6 と違います。★同じ数字で両方を縛ると、
     *    ★**動かしていないはずの旧台本まで動かす**ことになります。
     */
