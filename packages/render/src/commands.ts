...
export type PaletteRole =
   ...
   * ★**枠色**（`frame-1` 〜 `frame-8`）。
   *
   *   ⚠️ 以前は `silk-1` 〜 `silk-18` を**18色すべて別々**に持っていました。
   *      ★**単に頭数ぶん色を割り当てただけ**で、18頭立てでは
   *      「いちばん近い2色」が見分けられなくなります。
   *   → ★**実際の競馬の作法に寄せます: 色は枠、数字は個体**（D-060）。
   *      `bracketOf(馬番, 頭数)` で枠番を出し、色は 8色だけ使います。
   *   ★**必ず馬番と併記**すること（アートバイブル §4・色覚多様性）。
   */
  | `frame-${number}`
  /**
   * ★勝負服（個体）。
   *   ⚠️ **色の割り当てには使いません**（それは `frame-*`）。
   *      スプライトの色替えの参照先として残します。
   */
  | `silk-${number}`;
