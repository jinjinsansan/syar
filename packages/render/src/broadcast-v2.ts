...
const SHOTS: Readonly<Record<BroadcastV2ShotId, BroadcastV2Shot>> = {
  ...
  'fourth-corner-front': {
     ...
     */
    /**
     * ★**カメラを 90m 先 → 30m 先へ戻します**（2026-08-29・残件 A-1 の根治）
     *
     * 【★90m を選んだときの基準が、オーナーの苦情と違うものでした】
     *   ★2026-08-26 に 30m→90m にした理由は「★**カット内**の反転を消す」ことでした。
     *   ⚠️ ★しかしオーナーの苦情（「別のレースに見える」）は★**境目**の反転です。
     *   ★掃引表は**中央のコマの向き**と**カット内の反転回数**しか出しておらず、
     *     ★**境目で効く「入口の 1 コマ目」「出口の最終コマ」を一度も出していませんでした。**
     *   → ★**測っていない量を最適化していた**ことになります（D-082 と同じ形）。
     *
     * 【★入口・出口を出したら、90m は境目の反転が 2 箇所でした】
     *   ★前後はどちらも `side-drive`（→82px/m）なので、★入口・出口とも → なら境目の反転は 0 です。
     *
     ...
     *
     * 【★「カット内 1 回」は、むしろ正しい形です】
     *   ★映像編集で禁じられているのは**境目**で向きが変わること（イマジナリーラインを越える）。
     *   ★**カットの中でカメラの軸を跨ぐのは、線を越える正当なやり方**です。
     *   ⚠️ ★2026-08-25 にオーナーが却下したのは**素材の左右反転をカット開始時に固定する機構**
     *      （`aa52b70`）であって、★**画面上の向きが軸で入れ替わること自体ではありません。**
     *      ★前便はこの 2 つを同じものとして扱っていました。
     *
     * 【★ついでに戻るもの】
     *   ★90m では自動ズームが大きさを **21% で一定**に保っていました（＝迫ってこない）。
     *   ★30m なら **21% → 28%** と、`approach` に頼らずとも実際に大きくなります。
     */
    fixedCamera: {
      sFromSegmentEnd: 30, w: 22, upM: 7,
      /**
       * ★カット中のカメラまでの距離は **133m → 35m**（実測・`tools/_camdist.mjs`）。その範囲に合わせます。
       *   ⚠️ ★**範囲を実測から取ること。** 前便は 150m→55m と置いて、
       *      ★カットが終わるまでに伸び切らず **19.7% 止まり**でした（手置きの m を書かない）。
       */
 ...
 *   ★`true` にすると「**走線に沿った長さ**で N m 手前」に置きます。
