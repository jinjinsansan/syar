...
/**
 * ★**写真判定の札を出しておく長さ**（秒）。
 * ⚠️ ★**2026-09-15 に止め絵をやめました**（★オーナー判断「止めない」）。
 *    ★以前は ★この間 ★画面を先頭が線を通る瞬間で ★0.4 秒止め、★放すと 0.4 秒先へ跳んでいました。
 *    ★オーナー評「★なんかスピードが一瞬緩む、ような場面があります」（★流星大賞典・ハナ差）。
 *    ★実画面の通常再生で ★38.35〜38.74 秒の 0.38 秒、★表示が止まっていたのを確かめています。
 * → ★走りは流したまま、★札と決勝線の金の線だけを出します。★止めないので、★読める長さに延ばしました。
 */
export const PHOTO_FINISH_BADGE_SEC = 1.4;
