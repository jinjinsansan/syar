...
/**
 * ★**コーナーのカットの長さに足す「のりしろ」**（秒・★2026-09-12）。
 *
 *   ★コーナーへ入るところで出したカットインで、★**出口の継ぎ目まで覆う**ための余白です。
 *   ★実測（★台本 v8・seed 42）: ★コーナー→直線の切り替わりで ★カメラの視点が
 *   ★**14m 後ろへ跳び**、★勝ち馬が ★**1 コマで 57〜122px 左へ**動きます。
 *   ★その 1 コマがカットインの外に出ると、★芝の目が逆回転して見えます。
 */
 ...
 * ⚠️ ★**1.0 → 0.5 秒**（★2026-09-12）。★コーナーを全部見せる形にすると跳びが 3 か所になり、
 *    ★1.0 秒では ★**合わせて 10.0 秒**が挿入画面になりました（★実測）。★0.5 秒でも
 *    ★30fps で 15 コマあり、★1 コマの跳びを隠すには十分です。
 */
...
export function raceCutInAt(
  ...
  /**
   * ★**コーナーへ「入る」ところで出します**（★2026-09-12・★オーナー指示・★2 度目の改訂）
   *
   * 【★経緯 — ★出す場所を 2 回動かしています】
   *   ★1 回目（★2026-09-12 朝）… ★**コーナーから出る**ところ。
   *     ★オーナーの組み立て「コーナー演出（数秒）→ カットインで誤魔化す → 真横」に従いました。
   *   ★2 回目（★これ）… ★**コーナーへ入る**ところへ移します。
   *     ★オーナー評「★これも馬が全然違う方向を見ています。★おそらくこれ以上品質を
   *     ★上げるのは無理だと思うので ★**ここのシーンの馬が曲がってくる 2 秒くらいで
   *     ★またカットインを入れてください**。★そうすれば馬の方向は気にならないはずです」。
   *   → ★**コーナーそのものを覆います。** ★出口の継ぎ目（★カメラの視点が 14m 後退する）も
   *     ★同じ 1 枚で覆えるよう、★尺は ★**コーナーのカットの長さ＋のりしろ**を
   *     ★呼び出し側から渡します（★`RACE_CUTIN_SEAM_SEC`）。
   *
   * ⚠️ ★向きが合わないのは ★**素材が 3/4 正面の 1 ポーズ固定**だからで、★回転は
   *    ★オーナー指摘「馬が傾いています」で外しています。★素材の作り直しは 2 度不合格。
   *    ★＝ ★**隠すのが今いちばん確実な手**です。
   */
  if (!isCornerShot(fromId) && isCornerShot(toId)) {
    /** ★見出しは ★**画面が出している区間名**から（★カメラ名で断定しない・★R-30） */
    const here = opts.sectionLabel;
    const label = here === undefined || here === '' ? 'コーナー' : here;
