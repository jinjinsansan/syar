...
/**
 * ★砂煙の粒の数。★`perspective-draw.ts` の `PUFFS` の出どころ。
 *   ⚠️ ★下の `dustPlumePhaseAt` が**この数に依存**します。★2 か所で持つと位相がずれます（R-30）。
 */
export const DUST_PUFF_COUNT = 14;

/**
 * ★**砂煙の位相**（0〜1）を、★**進んだ距離から連続で**出す（2026-08-29）。
 *
 * ⚠️ 【★直した実害】★以前はここが `frameIndex`（＝**脚のコマ・8 段階**）から取られていました。
 ...
 *      ★砂粒は **8 → 75 → 80 → 12 → 75 → 80 px** と暴れていました（23 コマ中 **16 コマ**が 20px 超）。
 *      ★脚の位相は 1 コマで **2.87 バケット**進むので、位相が 2/8・3/8 と不揃いに跳んで巻き戻ります。
 *    ★オーナー評「★ダートのほうが遅く感じる／ぎこちない。芝は正常」（3 回目の苦情の真因）。
 *
 ...
 *
 * @param sM ★その馬が走路に沿って進んだ距離（m）
 */
export function dustPlumePhaseAt(sM: number): number {
  const x = ((sM * DUST_PUFF_COUNT) / DUST_PLUME_M) % 1;
  return (x + 1) % 1;
}
