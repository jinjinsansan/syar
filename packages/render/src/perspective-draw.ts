...
/**
 * ★**馬の絵を走路の接線へ回すか**（★既定・★2026-09-11・★オーナー判定）。
 *
 * 【★経緯】★オーナー評（★4 角）「★芝に対して ★**馬が斜め前を向いている**」。
 *   ★馬の絵は ★**画面に対してまっすぐ立つ板**で、★回りません。
 *   ★直線はカメラで解けました（★走路が画面で水平になるよう構え直した）。
 *   ⚠️ ★**コーナーは走路が曲がっている**ので、どう構えても弧のどこかで必ずずれます。
 *
 * 【★測った値】★馬（画面の水平）と芝の目がなす角（★90° が「馬の真横を芝の目が横切る」）:
 *   ★道中の真横 … 接線 **0°**／馬 0° … ★**一致**（★オーナー合格済み）
 *   ★4 コーナー … 接線 **−28°**／馬 0° … ★**28° ずれる**・交差角 52°
 *   ★4 コーナー・回すと … ★交差角 **80.5°**
 *   → ★オーナー判定「★**回したほうがまだマシ**」（★2026-09-11）。★既定をこちらにします。
 *
 * ⚠️ ★**合格ではありません。**「まだマシ」です。★素材（デフォルメの俯瞰）が要るという
 *    ★残件は動いていません（★`REPORT_P4_*` と `?asset=declared` の記録を参照）。
 * ⚠️ ★直線では `screenTrackAngle` がほぼ 0 を返すので、★**回しても 1 画素も動きません**。
 *    ★動くのは弧の上だけです（★検定で両方を留めています）。
 * ⚠️ ★戻し口は `/race?tilt=off`。★画面も道具も ★**この定数から引くこと**（★R-31）。
 */
...
export function drawPerspectiveHorses<TImage>(
  ...
  for (const d of drawn) {
     ...
     */
    const trackRot = (opts.alignToTrack ?? DEFAULT_ALIGN_TO_TRACK)
