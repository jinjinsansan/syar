...
export interface RaceBalance {
   ...
   * 着差→タイム変換の係数（正典 §8.7）。
   *
   * ★**スコアは順位を決める道具であって、物理量ではありません**（裁定 2026-08-13）。
   *   > 順位はスコアの大小で決まり、タイム差はその順位の見せ方にすぎません。
   *   > 両者の写像は独立に決められます。
   *
   * ⚠️ ここは**表示と記録にしか影響しません**。着順・V-4/V-5/V-6・払戻には触れません。
   */
  TIME_GAP_FACTOR: number;
  /**
   * ★**スコア差 → 着差の写像の「形」**（正典 §8.7 / D-064 の第三の自由度）
   *
   * 【何のためか】
   *   オーナー要望「直線で 4〜5 頭がせめぎ合ったままゴールさせたい」。
   *   実測（40 レース）では**ゴール時点で 4 頭以上が 2 馬身以内は 0%**、
   *   1着-2着の差の中央値 3.1 馬身、3 馬身以上の独走が 50% でした。
   *
   * 【なぜ較正（`RACE_RANDOM_K`）を触らないか】
   *   ★**スコアは順位を決める道具であって物理量ではありません**（D-064）。
   *   タイム差はその見せ方にすぎず、両者の写像は独立に決められます。
   *   現行はその写像が**線形**で、スコア差の広がりをそのまま着差に写しているだけです。
   *
   * 【形】`r_i = (S1 - Si)/S1` として
   *
   ...
   *
   *   - **レースごとの総差が定義から不変** ⇒ V-17② は動かない
   *   - **r について単調** ⇒ ★**着順は不変**（V-4/V-5/V-6・人気・払戻は定義上そのまま）
   *   - **γ = 1.0 が現行**（`race.ts` で現行の式へ短絡する）
   *
   * ⚠️ ★ここは**表示と記録にしか影響しません**。着順には触れません。
   * ⚠️ ★既定は 1.0 のままです。採用値はオーナー判定（`DEV_INSTRUCTIONS_P4_FINISH_CONTEST_20260825.md`）。
   */
  TIME_GAP_SHAPE_GAMMA: number;
}
 ...
 *
 * ─────────────────────────────────────────────
 * 🔴 ★**2026-09-19・オーナー決定（案 2）: 0.22 → 0.237**
 *
 *   ★出発点 ★**V-4 33.75%**（★`M-9` の後・★**8 シード**）。
 *   ★傾き ★**−72 pp/単位**（★**seed=42 の 1 本から**）。
 *   ★狙い ★**32.5%**（★帯 30〜34% の中央より少し下）。
 *
 * ⚠️ 🔴 ★**この 0.237 は外挿です。★測って出した値ではありません。**
 *   ★傾きは ★**1 シード**から採ったもので、★シード間のばらつきを見ていません。
 * ⚠️ 🔴 ★**「帯の外 0 本」は未測定です。** ★確定の 8 シードで**初めて**数えます
 *   （★オーナー決定②: ★平均で採るが、★帯の外の本数を必ず併記し、★0 でなければ余裕を足す）。
 *   ★動かす前は ★**V-4 が 2/8・V-5 が 1/8** 帯の外でした。
 *
 * ⚠️ ★**正典 §13.1 の `RACE_RANDOM_K` も同じ便で直すこと**（★**AU-3**・D-053 の先例）。
 *   ★較正で値が動いたら、★表も同じ便で。★片方だけ直すと、★今日 塞いだ穴が開き直ります。
 * ─────────────────────────────────────────────
 */
export const CALIBRATED_RACE_RANDOM_K = 0.237;
...
export const DEFAULT_RACE_BALANCE: RaceBalance = {
  ...
  /**
   * 🔴 ★**`1` → `0` に直しました**（★**M-9**・2026-09-19・オーナー決定の束 AL-11）。
   *
   * 【★何が食い違っていたか】
   *   ★正典 §7.4 と育成（`packages/training/src/condition.ts` の `CONDITION_RANGE`）は ★**0..5**。
   *   ★ここだけ ★**1..5** でした。→ ★`t = clamp((condition − 1) / 4, 0, 1)` なので
   *   ★★**調子 0 と 1 が、まったく同じ係数（0.88）**になっていました。
   *
   * 【🔴 ★そして、いちばん多い層でした】
   *   ✔ ★staging の現役 7,333 頭を数えた: ★調子 **0 が 30.53%**、★**1 が 31.77%**
   *   → ★★**62.30% が同じ係数を共有**していました（★「0 の馬はいないのでは」を疑って数えた結果）。
   *
   * 【⚠️ ★直す理由は「V-4 が動くから」ではありません】
   *   ✔ ★分解の実測（seed=42・15,000 レース）: ★**調子だけを実物にすると V-4 は −0.45pp（1.2 SE）**。
   *   → ★**単独では ±0.5pp の世界**です。★V-4 を 7.0 SE 動かしたのは ★**調子と疲労が一緒に動くこと**。
   *   → ★★**直す理由は「正典と育成は 0..5、対応表は 1..5」という食い違いの事実だけ**です。
   *
   * ⚠️ ★**6 段すべてが動きます**: ★調子 1 が +0.0440／2 が +0.0330／3 が +0.0220／4 が +0.0110
   *    （★0 と 5 は端なので動きません）。★実測の分布で加重すると ★集団平均 0.90389 → 0.92966（+2.85%）。
   * ⚠️ ★**測り直しは AL-11 の束で 1 回だけ**（★二度測らない）。★凍結プール `df10399b5c44` で。
   */
  CONDITION_MIN: 0,
  CONDITION_MAX: 5,
  ...
  /**
   * ★**0.55 → 0.045**（V-17 に合わせて較正・2026-08-14）
   *
   * 【何が起きていたか】
   *   1着と最下位の差が **35.0秒 ＝ ゴール時点で 415m** でした（実際の1600m戦は 2〜4秒）。
   *   ★能力・脚質・距離適性を**全部同一にしても 34.7秒**。能力差ではなく、
   *     **順位の不確実さを作る乱数を、そのままタイム差に写していた**ためです（二重に効いていた）。
   *
   * 【★決め打ちしていません】
   *   `tools/verify-v17.mjs` で掃引しました（1600m・200レース・差の p10/中央/p90）:
   *     0.55  → 25.74 / 33.06 / 44.63 秒   FAIL
   *     0.07  →  3.28 /  4.21 /  5.68 秒   FAIL（中央が 4秒を超える）
   *     0.05  →  2.34 /  3.01 /  4.06 秒   PASS（★p90 が 4秒をわずかに超える）
   *     0.045 →  2.11 /  2.70 /  3.65 秒   ★**PASS。p10〜p90 が 2〜4秒に完全に収まる**
   *     0.04  →  1.87 /  2.40 /  3.25 秒   PASS（ただし着差ラベルが潰れ始める）
   *     0.03  →  1.40 /  1.80 /  2.43 秒   FAIL（中央が 2秒を割る）
   *     0     →  0.00 /  0.00 /  0.00 秒   ★**FAIL（全馬同着＝別の破綻。下限が効いている）**
   *
   * 【★副作用も測りました】着差ラベルの分布（1600m・300レース）:
   *     0.55  → **3頭に1頭が「大差」**（33%）／8馬身 16%／5馬身 13%
   *     0.045 → クビ 12%／1/2馬身 14%／3/4馬身 14%／1 1/2馬身 13%
   *     0.04  → ハナ 13%／アタマ 13%（★今度は同着寄りに潰れ始める）
   *   ★**0.045 は、差の分布と着差ラベルの両方が同時に成り立つ値**です。
   */
  TIME_GAP_FACTOR: 0.045,
  /** ★既定は現行の線形（1.0）。1.3 / 1.6 は比較映像のためだけに道具側から差し替える */
  TIME_GAP_SHAPE_GAMMA: 1.0,
};
