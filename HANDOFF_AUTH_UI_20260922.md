...
レース常設表示の詳細は前日の引継ぎ書、`REPORT_RACE_NOTICE_FEASIBILITY_20260921.md`、既存のデザイン資料を参照。`RACE_NOTICE_HANDOFF.md` と `レース常設表示_設計案.dc.html` は**リポジトリ直下の `レースが数分おきに開催.zip` 内**の `design_handoff_uma_monogatari/` にある。通常の `rg --files` では ZIP の中身は見えない。2026-09-22 の後続調査で ZIP を開いて確認し、先の「見つからない」を訂正した。
