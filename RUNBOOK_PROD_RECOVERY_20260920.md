...
⚠️ **どうするかは決めていません**（→ 判断が要ります）:
- **案 A**: `races` / `race_odds` も消す（きれいになる。過去の記録は無くなる）
- **案 B**: 残す（履歴として。ただし**出走馬の無いレース**が画面に出うる）
- ⚠️ `races_public` は `race_entries` と結合するので、**案 B だと空のレースが並びます**
