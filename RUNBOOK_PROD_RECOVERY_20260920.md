...
| **④** | 🔴 **`data/ng-names.hash` が無い** ／ `verify-constitution.mjs` の **C-4 が不合格** | 🔴🔴 **④に進まない**（下記） |
...
- 🔴 **このまま ④ を流すと、また未検査の世界を作ります。** 弁護士に見せる前提が崩れます
- → **オーナーが `data/ng-names.txt` を置き、`npm run blocklist:build` を済ませてから ④**
  （`npx tsx tools/verify-constitution.mjs` が緑になること）
...
- **`race_entries` 79,746 行 が消えます**（★消さないと `horses` を消せないため）。
  → **本番の 6,066 レースの着順・オッズの突き合わせができなくなります**
