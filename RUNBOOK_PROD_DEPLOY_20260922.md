...
| 本番の未適用の移行 | `npx tsx tools/migrate.mjs --env production --plan` | 0060〜0070 が並ぶ。**0059 以前が並んだら止める**（前提が違う） |
...
## ④ 移行 `0066`〜最後（`0067`〜`0070` を含む）

```bash
npx tsx tools/migrate.mjs --env production --plan                     # 0066〜0070 の 5 件だけが並ぶこと
